...
suite("venvUtils", () => {
	...
	test("venvExists false", async () => {
		assert.equal(
			await venvUtils.venvExists("nonExistentPath", testFolder),
			false
		);
	...
	test("runCommandInVenv", async function (this: Mocha.Context): Promise<void> {
		...
		await venvUtils.runCommandInVenv(
			"python --version",
			venvName,
			testFolder
		);
	...
	test("convertToVenvCommand Windows powershell", async function (this: Mocha.Context): Promise<void> {
		...
		await runWithWindowsTerminal(
			"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe",
			() => {
				assert.equal(
					venvUtils.convertToVenvCommand(command, testFolder),
					".venv\\Scripts\\activate ; do a thing"
				);
				assert.equal(
					venvUtils.convertToVenvPythonCommand(
						command,
						venvName,
						"win32"
					),
					".venv\\Scripts\\python -m do a thing"
				);
			}
		);
	});
	...
	test("convertToVenvCommand Mac", async function (this: Mocha.Context): Promise<void> {
		...
		assert.equal(
			venvUtils.convertToVenvCommand(command, testFolder),
			". .venv/bin/activate && do a thing"
		);
		assert.equal(
			venvUtils.convertToVenvPythonCommand(command, venvName, "darwin"),
			".venv/bin/python -m do a thing"
		);
	});
	...
	test("convertToVenvCommand Linux", async function (this: Mocha.Context): Promise<void> {
		...
		assert.equal(
			venvUtils.convertToVenvPythonCommand(command, venvName, "linux"),
			".venv/bin/python -m do a thing"
		);
	});
});

async function runWithWindowsTerminal(
	terminalPath: string,
	callback: () => void
): Promise<void> {
	if (!(await AzExtFsExtra.pathExists(terminalPath))) {
		throw new Error(
			`Terminal path cannot be set because it does not exist: ${terminalPath}`
		);
	}
