...
suite("Create New Python Project", () => {
	test("skip venv", async () => {
		await runWithTestActionContext("createProject", async (context) => {
			await createAndValidateProject(context, {
				...getPythonValidateOptions(undefined, FuncVersion.v2),
				inputs: [/skip/i],
			});
		});
	});
	...
	test("enter venv", async function (this: Mocha.Context): Promise<void> {
		...
		const alias: string =
			process.platform === "win32" ? "py -3.7" : "python3.7";
		await runWithTestActionContext("createProject", async (context) => {
			await createAndValidateProject(context, {
				...getPythonValidateOptions(".venv", FuncVersion.v2),
				inputs: [/enter/i, alias],
			});
		});
	});
	...
	test("single existing venv", async () => {
		...
		await runWithTestActionContext("createProject", async (context) => {
			await createAndValidateProject(context, {
				...getPythonValidateOptions(venvName, FuncVersion.v2),
				projectPath,
			});
		});
	});
...
async function createTestVenv(
	projectPath: string,
	venvName: string
): Promise<void> {
	if (process.platform === "win32") {
		await AzExtFsExtra.ensureFile(
			path.join(projectPath, venvName, "Scripts", "activate")
		);
	} else {
		await AzExtFsExtra.ensureFile(
			path.join(projectPath, venvName, "bin", "activate")
		);
	}
}
