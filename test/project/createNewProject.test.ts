...
import {
	allTemplateSources,
	runForTemplateSource,
	shouldSkipVersion,
} from "../global.test";
import {
	createAndValidateProject,
	ICreateProjectTestOptions,
} from "./createAndValidateProject";
import {
...
const testCases: CreateProjectTestCase[] = [
	...
	{
		...getCSharpValidateOptions("netcoreapp3.1", FuncVersion.v3),
		inputs: [/3/],
	...
	},
	{
		...getCSharpValidateOptions("net6.0", FuncVersion.v4),
		inputs: [/6.*isolated/i],
		description: "net6.0 isolated",
	},
	{
		...getCSharpValidateOptions("net7.0", FuncVersion.v4),
		inputs: [/7.*isolated/i],
		description: "net7.0 isolated",
	},
	{
		...getFSharpValidateOptions("netcoreapp2.1", FuncVersion.v2),
		isHiddenLanguage: true,
	},
	{
		...getFSharpValidateOptions("netcoreapp3.1", FuncVersion.v3),
		inputs: [/3/],
		isHiddenLanguage: true,
	},
];
...
for (const version of [FuncVersion.v2, FuncVersion.v3, FuncVersion.v4]) {
	...
	const javaBaseInputs: (TestInput | string | RegExp)[] = [
	...
	if (version !== FuncVersion.v2) {
		// v2 doesn't support picking a java version
...
for (const testCase of testCases) {
	for (const source of allTemplateSources) {
		...
		parallelTests.push({
			...
			skip:
				shouldSkipVersion(testCase.version) ||
				(testCase.description === JavaBuildTool.gradle &&
					source !== TemplateSource.Backup),
				...
				await runWithTestActionContext(
					...
					async (context) => {
						await runForTemplateSource(
							...
							source,
							async () => {
								await createAndValidateProject(
									context,
									testCase
								);
							}
