...
suite("Init Project For VS Code", function (this: Mocha.Suite): void {
	...
	test("JavaScript", async () => {
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles: [["HttpTriggerJs", "index.js"]],
		});
	});

	test("JavaScript with package.json", async () => {
		await initAndValidateProject({
			...getJavaScriptValidateOptions(true /* hasPackageJson */),
			mockFiles: [["HttpTriggerJs", "index.js"], "package.json"],
		});
	});
	...
	test("TypeScript", async () => {
		await initAndValidateProject({
			...getTypeScriptValidateOptions({ missingCleanScript: true }),
			mockFiles: [
	...
	test("C#", async () => {
		...
		await initAndValidateProject({
			...getCSharpValidateOptions("netstandard2.0", FuncVersion.v2),
			mockFiles,
		});
	});
	...
	test("C# with extensions.csproj", async () => {
		...
		await initAndValidateProject({
			...getCSharpValidateOptions("netstandard2.0", FuncVersion.v2, 2),
			mockFiles,
		});
	});
	...
	test("Python no venv", async () => {
		...
		await initAndValidateProject({
			...getPythonValidateOptions(undefined),
			mockFiles,
			inputs: [/skip/i],
		});
	});
	...
	test("Python single venv", async () => {
		...
		const mockFiles: MockFile[] = [
			["HttpTrigger", "__init__.py"],
			"requirements.txt",
			getMockVenvPath(venvName),
		];
		await initAndValidateProject({
			...getPythonValidateOptions(venvName),
			mockFiles,
		});
	});
	...
	test("Python with extensions.csproj", async () => {
		const venvName: string = "testEnv";
		const options: IValidateProjectOptions =
			getPythonValidateOptions(venvName);
		options.expectedTasks.push("extensions install");
		options.expectedSettings["files.exclude"] = { obj: true, bin: true };
		options.expectedSettings["azureFunctions.preDeployTask"] =
	...
	test("F#", async () => {
		...
		await initAndValidateProject({
			...getFSharpValidateOptions("netstandard2.0", FuncVersion.v2),
			mockFiles,
		});
	});
    ...
    <properties>
		...
		await initAndValidateProject({
			...getJavaValidateOptions(appName, JavaBuildTool.maven),
			mockFiles,
		});
	});
    ...
    appName = '${appName}'
		...
		await initAndValidateProject({
			...getJavaValidateOptions(appName, JavaBuildTool.gradle),
			mockFiles,
		});
	});
	...
	test("Multi-language", async () => {
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles: [
				["HttpTriggerTS", "index.ts"],
				["HttpTriggerCSX", "run.csx"],
			],
			inputs: [ProjectLanguage.JavaScript],
		});
	});
	...
	test("Multi-function", async () => {
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles: [
				["HttpTriggerJS1", "index.js"],
				["HttpTriggerJS2", "index.js"],
			],
		});
	});
	...
	test("Existing extensions.json", async () => {
		const mockFiles: MockFile[] = [
			{
				fsPath: [".vscode", "extensions.json"],
				contents: { recommendations: ["testid"] },
			},
		];
		const options: IInitProjectTestOptions = {
			...getJavaScriptValidateOptions(),
			mockFiles,
			inputs: [ProjectLanguage.JavaScript],
		};
	...
	test("Invalid extensions.json", async () => {
		const mockFiles: MockFile[] = [
			{ fsPath: [".vscode", "extensions.json"], contents: "{" },
		];
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
			inputs: [ProjectLanguage.JavaScript, "Yes"],
		});
	});
		...
		const mockFiles: MockFile[] = [
			{
				...
				contents: { "azureFunctions.testSetting": "testValue" },
			},
		];
		const options: IInitProjectTestOptions = {
			...getJavaScriptValidateOptions(),
	...
	test("Invalid settings.json", async () => {
		const mockFiles: MockFile[] = [
			{ fsPath: [".vscode", "settings.json"], contents: "{" },
		];
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
	...
	test("Invalid tasks.json", async () => {
		const mockFiles: MockFile[] = [
			{ fsPath: [".vscode", "tasks.json"], contents: "{" },
		];
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
	...
	test("Overwrite existing task", async () => {
		...
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
			inputs: [ProjectLanguage.JavaScript, "Overwrite"],
		});
	});
	...
	test("Invalid launch.json", async () => {
		const mockFiles: MockFile[] = [
			{ fsPath: [".vscode", "launch.json"], contents: "{" },
		];
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
	...
	test("Invalid gitignore", async () => {
		const mockFiles: MockFile[] = [
			{ fsPath: ".gitignore", contents: ".vscode" },
		];
		await initAndValidateProject({
			...getJavaScriptValidateOptions(),
			mockFiles,
...
type MockFile =
	| MockFilePath
	| { fsPath: MockFilePath; contents?: string | object; isDir?: boolean };
...
async function initAndValidateProject(
	options: IInitProjectTestOptions
): Promise<void> {
	...
	mockFiles.push(
		...
		".funcignore",
		".gitignore",
		{ fsPath: ".git", isDir: true }
	);

	await Promise.all(
		mockFiles.map(async (mockFile) => {
			mockFile =
				typeof mockFile === "string" || Array.isArray(mockFile)
					? { fsPath: mockFile }
					: mockFile;

			const subPaths: string[] =
				typeof mockFile.fsPath === "string"
					? [mockFile.fsPath]
					: mockFile.fsPath;
			const fullPath: string = path.join(projectPath, ...subPaths);
			mockFile.isDir
				? await AzExtFsExtra.ensureDir(fullPath)
				: await AzExtFsExtra.ensureFile(fullPath);
