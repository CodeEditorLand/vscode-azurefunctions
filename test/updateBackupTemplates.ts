...
import {
	...
	FuncVersion,
	ProjectLanguage,
	supportedLanguages as resourceLanguages,
	...
	test("Update", async () => {
		...
		const workers: WorkerRuntime[] = [
			{ language: ProjectLanguage.JavaScript, versions: allVersions },
			{
				...
				versions: [FuncVersion.v1, FuncVersion.v2],
			},
			{
				language: ProjectLanguage.CSharp,
				projectTemplateKey: "netcoreapp3.1",
				versions: [FuncVersion.v3],
			},
			{
				language: ProjectLanguage.CSharp,
				projectTemplateKey: "net6.0",
				versions: [FuncVersion.v4],
			},
			{
				language: ProjectLanguage.CSharp,
			...
			},
			{
				...
				versions: [FuncVersion.v2, FuncVersion.v3, FuncVersion.v4],
			},
		];
			...
			for (const version of Object.values(FuncVersion)) {
				...
				for (const provider of providers) {
					...
					async function updateBackupTemplatesInternal(): Promise<void> {
						await provider.getLatestTemplates(
							context,
							templateVersion
						);
