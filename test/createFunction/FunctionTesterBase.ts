...
export abstract class FunctionTesterBase implements Disposable {
	...
	public async initAsync(): Promise<void> {
		await runWithTestActionContext(
			"testCreateFunctionInit",
			async (context) => {
				await runForTemplateSource(
					context,
					this.source,
					async (templateProvider) => {
			...
			async (context) => {
				await runForTemplateSource(
					...
					async (templateProvider) => {
						...
						assert.deepEqual(
							this.testedFunctions.sort(),
							templates.map((t) => t.name).sort(),
							'Not all "Verified" templates were tested'
						);
					}
				);
			}
	...
	public addParallelSuite(
		testCases: CreateFunctionTestCase[],
		options?: Partial<ParallelSuiteOptions>
	): void {
	...
	public async testCreateFunction(
		templateName: string,
		...inputs: string[]
	): Promise<void> {
			...
			async (context) => {
				try {
					await runForTemplateSource(
						...
						this.source,
						async () => {
							await this.testCreateFunctionInternal(
								context,
								this.projectPath,
								templateName,
								inputs.slice()
	...
	public async validateFunction(
		...
		for (const expectedPath of expectedPaths) {
			...
			assert.ok(
				await AzExtFsExtra.pathExists(filePath),
				`Failed to find expected path "${expectedPath}"`
			);
		}
		...
		const contents: string = (
			await AzExtFsExtra.readFile(mainFilePath)
		).toString();
		for (const expectedContent of expectedContents) {
			assert.ok(
				contents.includes(expectedContent) ||
					contents.includes(expectedContent.toLowerCase()),
				`Failed to find expected content "${expectedContent}" in "${mainFileName}"`
			);
		}
	}
	...
	protected async initializeTestFolder(testFolder: string): Promise<void> {
		...
		await Promise.all([
			...
			AzExtFsExtra.writeFile(
				path.join(testFolder, "local.settings.json"),
				'{ "Values": { "AzureWebJobsStorage": "test" } }'
			),
			AzExtFsExtra.writeFile(
				path.join(testFolder, ".vscode", "launch.json"),
				""
			),
		]);
	}

	private async testCreateFunctionInternal(
		...
		const funcName: string =
			templateName.replace(/[^a-z0-9]/gi, "") + getRandomHexString();
