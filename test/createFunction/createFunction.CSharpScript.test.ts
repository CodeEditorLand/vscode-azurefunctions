...
class CSharpScriptFunctionTester extends FunctionTesterBase {
	...
	protected override async initializeTestFolder(
		...
		const csprojContents: string = `<Project Sdk="Microsoft.NET.Sdk">
            <PropertyGroup>
		...
		await AzExtFsExtra.writeFile(
			path.join(testFolder, "project.csproj"),
			csprojContents
		);
	}
}
...
for (const source of allTemplateSources) {
	...
	suite(tester.suiteName, function (this: Mocha.Suite): void {
		...
		const iotTriggerSettings: {} = {
			connection: iotConnection,
			path: iotPath,
			consumerGroup: iotConsumerGroup,
		};
		const iotExpectedContents: string[] = [
			iotConnection,
			iotPath,
			iotConsumerGroup,
		];
