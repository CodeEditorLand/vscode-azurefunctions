...
abstract class NodeScriptFunctionTester extends FunctionTesterBase {
	protected override async initializeTestFolder(
		testFolder: string
	): Promise<void> {
...
class PythonFunctionTester extends FunctionTesterBase {
	...
	protected override async initializeTestFolder(
		...
		await AzExtFsExtra.writeFile(
			path.join(testFolder, "requirements.txt"),
...
function addSuite(tester: FunctionTesterBase): void {
	const testCases: CreateFunctionTestCase[] = [
		...
		},
		{
			...
			inputs: [],
			skip:
				tester.language === ProjectLanguage.PowerShell ||
				tester.language === ProjectLanguage.Java,
		},
		{
			functionName: "Durable Functions Entity HTTP starter",
			inputs:
				tester.language === ProjectLanguage.JavaScript
					? []
					: [getRotatingAuthLevel()],
			skip:
				tester.language === ProjectLanguage.PowerShell ||
				tester.language === ProjectLanguage.Java ||
				tester.language === ProjectLanguage.Python,
		},
		{
