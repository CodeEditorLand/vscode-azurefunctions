...
class CSharpFunctionTester extends FunctionTesterBase {
	...
	protected async initializeTestFolder(testFolder: string): Promise<void> {
		...
		await AzExtFsExtra.writeFile(
			path.join(testFolder, "test.csproj"),
    ...
    <ItemGroup>
        ...
        <PackageReference Include="${
			durableUtils.dotnetInProcDfBasePackage
		}" Version="2.9.2" />
    </ItemGroup>
</Project>`
		);
...
function addSuite(
	...
	const testCases: CreateFunctionTestCase[] = [
		...
		},
		{
			...
			inputs: ["TestCompany.TestFunction"],
			// skip: isIsolated
		},
		{
		...
		},
		{
			...
			inputs: ["TestCompany.TestFunction", getRotatingAuthLevel()],
		},
		{
		...
		},
		{
			...
			inputs: ["TestCompany.TestFunction", "0 * * * */6 *"],
		},
	];

	const tester: CSharpFunctionTester = new CSharpFunctionTester(
		version,
		targetFramework,
		source,
		!!isIsolated
			...
			if (version === FuncVersion.v2) {
				...
				test("createFunction API (deprecated)", async () => {
					...
					const templateId: string =
						"Azure.Function.CSharp.IotHubTrigger.2.x";
					...
					await tester.validateFunction(
						tester.projectPath,
						functionName,
						[namespace, iotPath, connection]
					);
				});
			}
		},
