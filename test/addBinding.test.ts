...
suite("Add Binding", () => {
	...
	suiteSetup(async () => {
		...
		functionJsonPath = path.join(
			testWorkspacePath,
			functionName,
			"function.json"
		);
		assert.ok(
			await AzExtFsExtra.pathExists(functionJsonPath),
	...
	suiteTeardown(async () => {
		...
		assert.equal(
			finalBindingsCount,
			initialBindingsCount /* 3
        https://github.com/microsoft/vscode-azurefunctions/issues/3266 */ + 1,
			"Not all expected bindings were added."
		);
	});
	...
	test("Tree", async function (this: Mocha.Context): Promise<void> {
		// https://github.com/microsoft/vscode-azurefunctions/issues/3266
		this.skip();
		const treeItem: AzExtTreeItem | undefined =
			await ext.rgApi.workspaceResourceTree.findTreeItem(
				`/localProject0/functions/${functionName}`,
	...
	async function getBindingsCount(): Promise<number> {
		const data: IFunctionJson =
			await AzExtFsExtra.readJSON<IFunctionJson>(functionJsonPath);
		return (data.bindings || []).length;
	}

	async function validateAddBinding(
		commandInput: any,
		userInputs: string[]
	): Promise<void> {
		...
		await runWithTestActionContext("addBinding", async (context) => {
			await context.ui.runWithInputs(
				[...userInputs, bindingDirection, bindingType, bindingName],
				async () => {
		...
		const data: IFunctionJson =
			await AzExtFsExtra.readJSON<IFunctionJson>(functionJsonPath);
		const binding: IFunctionBinding | undefined =
			data.bindings && data.bindings.find((b) => b.name === bindingName);
		if (!binding) {
