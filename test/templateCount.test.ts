...
import {
	...
	longRunningTestsEnabled,
	runForTemplateSource,
	shouldSkipVersion,
	skipStagingTemplateSource,
} from "./global.test";
...
function addSuite(source: TemplateSource | undefined): void {
	suite(
		`Template Count - ${
			source === undefined ? "defaultOnExtensionActivation" : source
		}`,
		() => {
			const cases: TestCase[] = [
				...
				},
				{
					...
					version: FuncVersion.v3,
					expectedCount: 12,
					projectTemplateKey: "netcoreapp3.1",
				},
				{
					language: ProjectLanguage.CSharp,
					version: FuncVersion.v4,
					expectedCount: 12,
					projectTemplateKey: "net6.0",
				},
				{
					language: ProjectLanguage.CSharp,
					version: FuncVersion.v4,
					expectedCount: 10,
					projectTemplateKey: "net6.0-isolated",
				},
				{
					language: ProjectLanguage.CSharp,
					version: FuncVersion.v4,
					expectedCount: 10,
					projectTemplateKey: "net7.0-isolated",
				},
				{
						...
						await runWithTestActionContext(
							...
							async (context) => {
								await runForTemplateSource(
									...
									source,
									async (
										provider: CentralTemplateProvider
									) => {
										const templates: FunctionTemplateBase[] =
											await provider.getFunctionTemplates(
												context,
												testWorkspacePath,
												language,
												undefined,
