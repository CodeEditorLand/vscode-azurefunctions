...
suite(`AzureFunctionsExtensionApi`, () => {
	...
	suiteSetup(() => {
		const extension:
			| Extension<apiUtils.AzureExtensionApiProvider>
			| undefined = extensions.getExtension(extensionId);
		api =
			nonNullValue(extension).exports.getApi<AzureFunctionsExtensionApi>(
				"^1.0.0"
			);
	});
	...
	test("createFunction in a subfolder of a workspace", async () => {
		...
		await runWithInputs(
			"api.createFunction",
			[language, /Model V3/, functionName],
			registerOnActionStartHandler,
			async () => {
		...
		const validateOptions: IValidateProjectOptions =
			getJavaScriptValidateOptions(
				true,
				undefined,
				projectSubpath,
