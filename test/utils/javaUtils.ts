...
export namespace javaUtils {
	export async function addJavaProjectToWorkspace(
		...
		if (
			!(await AzExtFsExtra.pathExists(
				path.join(testWorkspacePath, "pom.xml")
			))
		) {
			// no need if the project is already created
			const inputs: (string | TestInput | RegExp)[] = [
				testWorkspacePath,
