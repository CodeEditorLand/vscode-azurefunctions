 ...
 */
export const testFolderPath: string = path.join(
	os.tmpdir(),
	`azFuncTest${getRandomHexString()}`
);

export const longRunningTestsEnabled: boolean =
	envUtils.isEnvironmentVariableSet(process.env.ENABLE_LONG_RUNNING_TESTS);
export const updateBackupTemplates: boolean = envUtils.isEnvironmentVariableSet(
	process.env.AZFUNC_UPDATE_BACKUP_TEMPLATES
);
export const skipStagingTemplateSource: boolean =
	envUtils.isEnvironmentVariableSet(process.env.SKIP_STAGING_TEMPLATE_SOURCE);
...
export function getTestWorkspaceFolder(): string {
	if (workspaceFolderIndex >= testWorkspaceFolders.length) {
		throw new Error(
			'Not enough workspace folders. Add more in "test/test.code-workspace".'
		);
	}
...
suiteSetup(async function (this: Mocha.Context): Promise<void> {
	...
	const funcExtension = vscode.extensions.getExtension(
	...
	ext.defaultFuncCliPath =
		process.env.FUNC_PATH ||
		path.join(os.homedir(), "tools", "func", "func");
...
suiteTeardown(async function (this: Mocha.Context): Promise<void> {
	...
	try {
		...
		console.warn(
			`Failed to delete test folder path: ${parseError(error).message}`
		);
	}
...
async function preLoadTemplates(): Promise<void> {
	const providers = [
		ext.templateProvider.get(await createTestActionContext()),
	];
		...
		await runWithTestActionContext("preLoadTemplates", async (context) => {
			...
			for (const version of Object.values(FuncVersion)) {
				...
				for (const language of [
					...
					tasks.push(
						provider.getFunctionTemplates(
							context,
							testWorkspaceFolders[0],
							language,
							undefined,
							version,
...
export function isLongRunningVersion(version: FuncVersion): boolean {
	return (
		...
		version === FuncVersion.v2 ||
		version === FuncVersion.v3
	);
}
...
export const allTemplateSources: TemplateSource[] =
	Object.values(TemplateSource);
export async function runForTemplateSource(
	context: IActionContext,
	source: TemplateSource | undefined,
	callback: (templateProvider: CentralTemplateProvider) => Promise<void>
): Promise<void> {
	...
	if (!workspaceFolders) {
		...
		for (let i = 0; i < workspaceFolders.length; i++) {
			...
			assert.equal(
				folderName,
				String(i),
				`Unexpected workspace folder name "${folderName}".`
			);
