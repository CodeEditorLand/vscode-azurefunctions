...
import {
	...
	validateProject,
} from "../project/validateProject";
import {
	getRotatingAuthLevel,
	getRotatingLocation,
	getRotatingNodeVersion,
	getRotatingPythonVersion,
} from "./getRotatingValue";
import { resourceGroupsToDelete } from "./global.nightly.test";

interface CreateProjectAndDeployTestCase
	extends ICreateProjectAndDeployOptions {
	title: string;
...
const testCases: CreateProjectAndDeployTestCase[] = [
	{
		title: "JavaScript",
		...getJavaScriptValidateOptions(true),
		deployInputs: [getRotatingNodeVersion()],
	},
	{
		title: "TypeScript",
		...getTypeScriptValidateOptions(),
	...
	// All C# tests on mac and .NET 6 on windows are consistently timing out for some unknown reason. Will skip for now
	{
		...
		buildMachineOsToSkip: "darwin",
		...getCSharpValidateOptions("netcoreapp3.1"),
		createProjectInputs: [/net.*3/i],
		deployInputs: [/net.*3/i],
		createFunctionInputs: ["Company.Function"],
	},
	{
		title: "C# .NET 6",
		buildMachineOsToSkip: ["darwin", "win32"],
		...getCSharpValidateOptions("net6.0", FuncVersion.v4),
		createProjectInputs: [/net.*6/i],
		deployInputs: [/net.*6/i],
		createFunctionInputs: ["Company.Function"],
	},
	{
		title: "PowerShell",
		...getPowerShellValidateOptions(),
		deployInputs: [/powershell.*7/i],
	},
	{
		title: "Python",
		buildMachineOsToSkip: "win32",
		...getPythonValidateOptions(".venv"),
		createProjectInputs: [/3\.7/],
		deployInputs: [getRotatingPythonVersion()],
	},
];

const parallelTests: ParallelTest[] = [];
for (const testCase of testCases) {
	const osToSkip = Array.isArray(testCase.buildMachineOsToSkip)
		? testCase.buildMachineOsToSkip
		: [testCase.buildMachineOsToSkip];
...
async function testCreateProjectAndDeploy(
	options: ICreateProjectAndDeployOptions
): Promise<void> {
	...
	await runWithTestActionContext("createNewProject", async (context) => {
		...
		await context.ui.runWithInputs(
			[
				testWorkspacePath,
				options.language,
				...options.createProjectInputs,
				/http\s*trigger/i,
				functionName,
				...options.createFunctionInputs,
				getRotatingAuthLevel(),
			],
			async () => {
				const createProjectOptions =
					options.version !== defaultTestFuncVersion
	...
	await runWithTestActionContext("deploy", async (context) => {
		...
		await context.ui.runWithInputs(
			[
				testWorkspacePath,
				/create new function app/i,
				appName,
				...options.deployInputs,
				getRotatingLocation(),
			],
			async () => {
...
async function addRoutePrefixToProject(
...
async function validateFunctionUrl(
	...
	assert.ok(
		functionUrl?.includes(routePrefix),
		`Function url "${functionUrl}" did not include routePrefix "${routePrefix}".`
	);
	...
	assert.ok(
		(body.includes("Hello") && body.includes("World")) ||
			body.includes("Welcome"),
		'Expected function response to include "Hello World" or "Welcome"'
	);
}
