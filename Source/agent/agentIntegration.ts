...
import {
	...
	type AzureUserInputQueue,
	type IAzureUserInput,
	type SimpleCommandConfig,
	type WizardCommandConfig,
...
export async function getCommands(): Promise<
	(WizardCommandConfig | SimpleCommandConfig)[]
> {
	return [
		{
			type: "wizard",
			...
			intentDescription:
				"This is best when users ask to create a Function App resource in Azure. They may refer to a Function App as 'Function App', 'function', 'function resource', 'function app resource', 'function app' etc. This command is not useful if the user is asking how to do something, or if something is possible.",
		...
		},
		{
			...
			intentDescription:
				"This is best when users ask to create a new function project in VS Code. They may also refer to creating a function project by asking to create a project based upon a function project template.",
			requiresAzureLogin: true,
		},
		{
			type: "simple",
			...
			intentDescription:
				"This is best when users ask to create a deploy their function project or their code to a function app. They may refer to a Function App as 'Function App', 'function', 'function resource', 'function app resource', 'function app' etc. This is not best if they ask to deploy to a slot.",
			requiresAzureLogin: true,
		},
	];
}

export async function runWizardCommandWithoutExecution(
	command: WizardCommandConfig,
	ui: IAzureUserInput,
): Promise<void> {
	if (command.commandId === "azureFunctions.createFunctionApp") {
		await callWithTelemetryAndErrorHandling(
			"azureFunctions.createFunctionAppViaAgent",
			async (context) => {
...
export async function runWizardCommandWithInputs(
	command: WizardCommandConfig,
	inputsQueue: AzureUserInputQueue,
): Promise<void> {
	if (command.commandId === "azureFunctions.createFunctionApp") {
		await callWithTelemetryAndErrorHandling(
			"azureFunctions.createFunctionAppViaAgent",
			async (context) => {
				const azureUserInput = new AzExtUserInputWithInputQueue(
					context,
					inputsQueue,
				);
...
function getNumericallyLabeledBenchmarkConfig(
...
function getLearnBenchmarkConfigs(): AgentBenchmarkConfig[] {
	return [
		...
		getNumericallyLabeledBenchmarkConfig(config, index + 1),
	);
}
