...
import {
	getFunctionAppName,
	getJavaDebugSubpath,
} from "../commands/initProjectForVSCode/InitVSCodeStep/JavaInitVSCodeStep";
import {
	funcVersionSetting,
	javaBuildTool,
	ProjectLanguage,
...
export type LocalProjectInternal = {
	options: LocalProjectOptionsInternal;
} & IProjectTreeItem;
...
async function getCompiledProjectInfo(
	context: IActionContext,
	projectPath: string,
	projectLanguage: ProjectLanguage,
): Promise<CompiledProjectInfo | undefined> {
	if (
		...
		if (projFiles.length === 1) {
			...
			return {
				compiledProjectPath: path.join(
					projectPath,
					dotnetUtils.getDotnetDebugSubpath(targetFramework),
				),
				isIsolated,
			};
		} else {
			throw new Error(
				localize("unableToFindProj", "Unable to detect project file."),
			);
		}
	} else if (projectLanguage === ProjectLanguage.Java) {
		const buildTool: JavaBuildTool | undefined = getWorkspaceSetting(
			javaBuildTool,
			projectPath,
		);
		...
		if (!functionAppName) {
			throw new Error(
				localize(
					"unableToGetFunctionAppName",
					'Unable to detect property "functionAppName" in pom.xml.',
				),
			);
		} else {
			return {
				compiledProjectPath: path.join(
					projectPath,
					getJavaDebugSubpath(functionAppName, buildTool),
				),
				isIsolated: false,
			};
		}
