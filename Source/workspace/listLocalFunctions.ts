...
import {
	...
	getFunctionFolders,
} from "../tree/localProject/LocalFunctionsTreeItem";
import { nonNullValue } from "../utils/nonNull";
import { isNodeV4Plus, isPythonV2Plus } from "../utils/programmingModelUtils";
import { requestUtils } from "../utils/requestUtils";
import { getWorkspaceSetting } from "../vsCodeConfig/settings";
import { type ILocalFunction, LocalFunction } from "./LocalFunction";
 ...
 */
export async function listLocalFunctions(
	project: LocalProjectInternal,
	...
	return (await callWithTelemetryAndErrorHandling(
		...
		async (context) => {
			...
			const isFunctionalProgrammingModel =
				isPythonV2Plus(
					project.options.language,
					project.options.languageModel,
				) || isNodeV4Plus(project.options);

			if (project.options.isIsolated || isFunctionalProgrammingModel) {
				return {
					functions: await getFunctionsForHostedProject(
						context,
						project,
					),
					invalidFunctions: [],
				...
				const functions: string[] = await getFunctionFolders(
					context,
					project.options.effectiveProjectPath,
				);
				for (const func of functions) {
					try {
						...
						const config: ParsedFunctionJson =
...
function getHostStartTimeoutMS(): number {
	...
	return (
		nonNullValue(getWorkspaceSetting<number>(timeoutKey), timeoutKey) * 1000
	);
}
 ...
 */
async function getFunctionsForHostedProject(
	context: IActionContext,
	project: LocalProjectInternal,
): Promise<ILocalFunction[]> {
	if (runningFuncTaskMap.has(project.options.folder)) {
		...
		while (retry) {
			...
			try {
				functions = await requestUtils.sendRequestWithExtTimeout(
					context,
					{
		...
		if (functions !== undefined) {
			return (<FunctionEnvelope[]>functions.parsedBody).map((func) => {
				...
				return new LocalFunction(
					project,
					nonNullProp(func, "name"),
					new ParsedFunctionJson(func.config),
					func,
				);
			});
		}
