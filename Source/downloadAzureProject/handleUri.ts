...
export async function handleUri(uri: vscode.Uri): Promise<void> {
	await callWithTelemetryAndErrorHandling(
		"azureFunctions.handleUri",
		async (context: IActionContext) => {
			const enableOpenFromPortal: boolean | undefined =
				getWorkspaceSetting<boolean>("enableOpenFromPortal");
			...
			if (enableOpenFromPortal) {
				...
				if (
					action === HandleUriActions.downloadContentAndSetupProject
				) {
				...
				throw new Error(
					localize(
						"featureNotSupported",
						"Download content and setup project feature is not supported for this extension version."
					)
				);
			}
		}
...
export function getRequiredQueryParameter(
	...
	if (value && typeof value === "string") {
		...
		throw new Error(
			localize("missingQueryParam", 'Missing query parameter "{0}".', key)
		);
	}
}
