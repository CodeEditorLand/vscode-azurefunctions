...
export async function setupProjectFolder(
	...
	try {
		...
		await vscode.window.withProgress(
			{
				...
				title: localize(
					"settingUpFunctionAppLocalProjInfoMessage",
					`Setting up project for function app '${functionAppName}' with language '${language}'.`
				),
			},
			async () => {
				// NOTE: We don't want to download app content for compiled languages.
				const slotTreeItem: SlotTreeItem | undefined =
					await ext.rgApi.tree.findTreeItem(resourceId, {
						...context,
						loadAll: true,
					});
				if (!slotTreeItem) {
					throw new Error(
						localize(
							"failedToFindApp",
							'Failed to find function app with id "{0}"',
							resourceId
						)
					);
				}
				...
				if (!!hostKeys && hostKeys.masterKey && defaultHostName) {
					...
					throw new Error(
						localize(
							"hostInformationNotFound",
							"Failed to get host information for the functionApp."
						)
					);
				}

				const projectFilePathUri: vscode.Uri = vscode.Uri.joinPath(
					vsCodeFilePathUri,
					`${functionAppName}`
				);
		...
		throw new Error(
			localize(
				"failedLocalProjSetupErrorMessage",
				'Failed to set up your local project: "{0}".',
				parseError(err).message
			)
		);
	} finally {
		await AzExtFsExtra.deleteResource(toBeDeletedFolderPathUri.fsPath, {
			recursive: true,
			useTrash: true,
		});
	}
