...
import { type ProjectLanguage, deploySubpathSetting } from "../constants";
import { localize } from "../localize";
import { dotnetUtils } from "../utils/dotnetUtils";
import { getWorkspaceSetting, updateWorkspaceSetting } from "./settings";
import { type ITask, getTasks, updateTasks } from "./tasks";
...
export async function verifyTargetFramework(
	...
	if (getWorkspaceSetting<boolean>(settingKey)) {
		const projFiles: dotnetUtils.ProjectFile[] =
			await dotnetUtils.getProjFiles(
				context,
				projectLanguage,
				projectPath,
			);
		if (projFiles.length === 1) {
			...
			try {
				targetFramework = await dotnetUtils.getTargetFramework(
					projFiles[0],
				);
			} catch {
				// ignore
			...
			const tasksResult: IVerifyFrameworkResult | undefined =
				verifyTasksFramework(folder, targetFramework);
			const settingsResult: IVerifyFrameworkResult | undefined =
				verifySettingsFramework(folder.uri.fsPath, targetFramework);

			const mismatchTargetFramework: string | undefined =
				(tasksResult && tasksResult.mismatchTargetFramework) ||
				(settingsResult && settingsResult.mismatchTargetFramework);
			if (mismatchTargetFramework) {
				context.telemetry.properties.verifyConfigPrompt =
...
function verifyTasksFramework(
	folder: vscode.WorkspaceFolder,
	projTargetFramework: string,
): IVerifyFrameworkResult | undefined {
	let mismatchTargetFramework: string | undefined;

	const tasks: ITask[] = getTasks(folder);
	for (const task of tasks) {
		if (task.options && task.options.cwd) {
			const matches: RegExpMatchArray | null = task.options.cwd.match(
				targetFrameworkRegExp,
			);
			const targetFramework: string | null = matches && matches[0];
			if (
				targetFramework &&
				targetFramework.toLowerCase() !==
...
function verifySettingsFramework(
	workspacePath: string,
	projTargetFramework: string,
): IVerifyFrameworkResult | undefined {
	let deploySubPath: string | undefined = getWorkspaceSetting(
		deploySubpathSetting,
		workspacePath,
	);
	if (deploySubPath) {
		const matches: RegExpMatchArray | null = deploySubPath.match(
			targetFrameworkRegExp,
		);
		const targetFramework: string | null = matches && matches[0];
		if (
			targetFramework &&
			targetFramework.toLowerCase() !== projTargetFramework.toLowerCase()
		) {
