...
export function getDebugConfigs(folder: WorkspaceFolder): DebugConfiguration[] {
	return (
		getLaunchConfig(folder).get<DebugConfiguration[]>(configurationsKey) ||
		[]
	);
}

export async function updateDebugConfigs(
...
export async function updateLaunchVersion(
	folder: WorkspaceFolder,
	version: string,
): Promise<void> {
...
export function isDebugConfigEqual(
	c1: DebugConfiguration,
	c2: DebugConfiguration,
): boolean {
	return (
		c1.name === c2.name &&
		c1.request === c2.request &&
		isTypeEqual(c1.type, c2.type)
	);
}
