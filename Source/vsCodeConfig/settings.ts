 ...
 */
export function getRootFunctionsWorkerRuntime(
...
export async function tryGetFunctionsWorkerRuntimeForProject(
...
export function isKnownWorkerRuntime(runtime: string | undefined): boolean {
	return (
		...
		[
			...
			"powershell",
			"custom",
		].includes(runtime.toLowerCase())
	);
}

export function promptToUpdateDotnetRuntime(
	azureRuntime: string | undefined,
	localRuntime: string | undefined,
): boolean {
	return (
		(azureRuntime === "dotnet" && localRuntime === "dotnet-isolated") ||
		(azureRuntime === "dotnet-isolated" && localRuntime === "dotnet")
	);
}

export function getFuncWatchProblemMatcher(
	language: string | undefined,
): string {
