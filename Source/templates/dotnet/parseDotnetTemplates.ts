...
import {
	ProjectLanguage,
	TemplateFilter,
	sqlBindingTemplateRegex,
} from "../../constants";
import { ext } from "../../extensionVariables";
import { localize } from "../../localize";
import { assertTemplateIsV1 } from "../../utils/templateVersionUtils";
import { type IBindingSetting, ValueType } from "../IBindingTemplate";
import {
...
function parseDotnetTemplate(rawTemplate: IRawTemplate): IFunctionTemplate {
	...
	return {
		isHttpTrigger:
			/^http/i.test(rawTemplate.Name) ||
			/webhook$/i.test(rawTemplate.Name),
		...
		language: /FSharp/i.test(rawTemplate.Identity)
		...
		isDynamicConcurrent:
			rawTemplate.Identity.includes("ServiceBusQueueTrigger") ||
			rawTemplate.Identity.includes("BlobTrigger") ||
			rawTemplate.Identity.includes("QueueTrigger")
				? true
				: false,
 ...
 */
export async function parseDotnetTemplates(
	rawTemplates: object[],
	version: FuncVersion,
): Promise<ITemplates> {
	...
	for (const rawTemplate of rawTemplates) {
		try {
			functionTemplates.push(
				parseDotnetTemplate(<IRawTemplate>rawTemplate),
			);
		} catch (error) {
 ...
 */
async function copyCSharpSettingsFromJS(
	csharpTemplates: IFunctionTemplate[],
	version: FuncVersion,
): Promise<void> {
		...
		async (jsContext: IActionContext) => {
			...
			for (const csharpTemplate of csharpTemplates) {
				...
				const jsTemplate: FunctionTemplateBase | undefined =
					jsTemplates.find(
						(t: IFunctionTemplate) =>
							normalizeScriptId(t.id) === normalizedDotnetId,
					);
