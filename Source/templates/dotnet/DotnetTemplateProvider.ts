...
export class DotnetTemplateProvider extends TemplateProviderBase {
	...
	public async refreshProjKey(context: IActionContext): Promise<string> {
		return await dotnetUtils.getTemplateKeyFromProjFile(
			context,
			this.projectPath,
			this.version,
			this.language
	...
	public async getLatestTemplateVersion(
		context: IActionContext
	): Promise<string> {
		const projKey = await this.getProjKey(context);

		const templateVersion = await cliFeedUtils.getLatestVersion(
			context,
			this.version
		);
		...
		if (netRelease) {
			...
			const templateVersions = await cliFeedUtils.getSortedVersions(
				context,
				this.version
			);
				...
				try {
					if (newFuncVersion !== this.version) {
						const newTemplateVersion =
							await cliFeedUtils.getLatestVersion(
								context,
								newFuncVersion
							);
						netRelease = await this.getNetRelease(
			...
			throw new Error(
				localize(
					"projKeyError",
					'"{0}" is not supported for Azure Functions version "{1}".',
					projKey,
					this.version
				)
			);
	...
	public async getLatestTemplates(
		...
		await Promise.all([
			requestUtils.downloadFile(
				context,
				netRelease.projectTemplates,
				projectFilePath
			),
			requestUtils.downloadFile(
				context,
				netRelease.itemTemplates,
				itemFilePath
			),
		]);
	...
	public includeTemplate(
		...
		return (
			...
			isIsolated(template.id) === isIsolated(this._sessionProjKey)
		);
	}
}
