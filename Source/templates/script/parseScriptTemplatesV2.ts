...
import type { ActionType, ProjectLanguage } from "../../constants";
...
export function parseScriptTemplates(
	rawTemplates: RawTemplateV2[],
	rawBindings: object[],
	resources: Resources,
): FunctionV2Template[] {
	const userPrompts: RawUserPrompt[] = parseUserPrompts(
		rawBindings,
		resources,
	);
...
export function parseUserPrompts(
	rawUserPrompts: object[],
	resources: Resources,
): RawUserPrompt[] {
	...
	for (const rawUserPrompt of rawUserPrompts) {
		...
		for (const key of Object.keys(rawUserPrompt)) {
			...
			if (typeof paramName === "string" && paramName.startsWith("$")) {
				...
				key === "validators" &&
				Array.isArray(rawUserPrompt[key])
			) {
				...
				for (const validator of validators) {
					...
					const matches: RegExpMatchArray | null =
						validator.errorText.match(/\[variables\(\'(.*)\'\)\]/);
					// but in the resources key is $variables_paramName
					validator.errorText =
						getResourceValue(
							resources,
							matches
								? "$variables_" + matches[1]
								: validator.errorText,
							true,
