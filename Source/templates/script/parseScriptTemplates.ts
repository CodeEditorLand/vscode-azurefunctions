...
import {
	ParsedFunctionJson,
	type IFunctionBinding,
} from "../../funcConfig/function";
import { localize } from "../../localize";
import {
	type IBindingSetting,
	type IBindingTemplate,
	type IEnumValue,
	type ResourceType,
	type ValueType,
} from "../IBindingTemplate";
import {
			...
			if (rawSetting.validators) {
				for (const validator of rawSetting.validators) {
					if (!value || value.match(validator.expression) === null) {
						return replaceHtmlLinkWithMarkdown(
							getVariableValue(
								resources,
								variables,
								validator.errorText,
...
export function parseScriptBindings(
	config: IConfig,
	resources: IResources,
): IBindingTemplate[] {
	...
	if (config.bindings) {
		for (const rawBinding of config.bindings) {
			try {
				if (rawBinding.type) {
					const settings: IBindingSetting[] = (
						rawBinding.settings || []
					).map((setting: object) =>
						parseScriptSetting(
							setting,
...
export function parseScriptTemplate(
...
): IScriptFunctionTemplate | undefined {
	if (!rawTemplate.id || !rawTemplate.metadata) {
		return undefined;
	}

	const functionJson: ParsedFunctionJson = new ParsedFunctionJson(
	...
	return {
		...
		isDynamicConcurrent:
			rawTemplate.id.includes("ServiceBusQueueTrigger") ||
			rawTemplate.id.includes("BlobTrigger") ||
			rawTemplate.id.includes("QueueTrigger")
				? true
				: false,
 ...
 */
export function parseScriptTemplates(
	rawResources: object,
	rawTemplates: object[],
	rawConfig: object,
): ITemplates {
	const bindingTemplates: IBindingTemplate[] = parseScriptBindings(
		<IConfig>rawConfig,
