...
export function getScriptVerifiedTemplateIds(
	version: string,
): (string | RegExp)[] {
	...
	if (version === FuncVersion.v1) {
		...
		const regExps = verifiedTemplateIds.map(
			(t) =>
				new RegExp(
					`^${t}-(JavaScript(${nodeV4Suffix})?|TypeScript(${nodeV4Suffix})?|Python|PowerShell|Custom|Python-Preview|Python-Preview-Append)$`,
					"i",
				),
		);
		...
		const entityRegExps = [
			new RegExp(
				`^DurableFunctionsEntity-(JavaScript(${nodeV4Suffix})?|TypeScript(${nodeV4Suffix})?|Python|Custom)$`,
				"i",
			),
			new RegExp(
				`^DurableFunctionsEntityHttpStart-(JavaScript(${nodeV4Suffix})?|TypeScript(${nodeV4Suffix})?|Custom)$`,
				"i",
			),
		];
