...
import { type IBundleMetadata, parseHostJson } from "../../funcConfig/host";
...
export class ScriptBundleTemplateProvider extends ScriptTemplateProvider {
	...
	public async getLatestTemplates(
		...
		[this._rawBindings, this._rawResources, this._rawTemplates] = <
			[object, object, object[]]
		>await Promise.all(
			urls.map((url) => feedUtils.getJsonFeed(context, url)),
		);
	...
	public async getBackupTemplates(): Promise<ITemplates> {
		...
		if (
			!bundleMetadata ||
			!bundleMetadata.id ||
			bundleMetadata.id === bundleFeedUtils.defaultBundleId
		) {
			...
			throw new Error(
				localize(
					"noBundleBackup",
					'Backup templates are not supported for bundle "{0}".',
					bundleMetadata.id,
				),
			);
		}
