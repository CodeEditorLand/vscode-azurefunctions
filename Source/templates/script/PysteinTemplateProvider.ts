...
import {
	parseScriptTemplates,
	type RawTemplateV2,
} from "./parseScriptTemplatesV2";
...
export class PysteinTemplateProvider extends ScriptBundleTemplateProvider {
	...
	public async getLatestTemplates(
		...
		const release: bundleFeedUtils.ITemplatesReleaseV2 =
			await bundleFeedUtils.getReleaseV2(
				context,
				bundleMetadata,
		...
		[this._rawBindings, this._resources, this._rawTemplates] = <
			[object[], { en: { [key: string]: string } }, RawTemplateV2[]]
		>await Promise.all(
			urls.map((url) => feedUtils.getJsonFeed(context, url)),
		);

		return {
			functionTemplates: parseScriptTemplates(
				this._rawTemplates,
				this._rawBindings,
				this._resources,
			),
	...
	public includeTemplate(
		...
		if (verifyTemplateIsV2(template)) {
			return (
				...
				ProjectLanguage.Python.toLowerCase()
			);
		}
