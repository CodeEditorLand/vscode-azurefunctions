...
import { type ConfigurationChangeEvent, Disposable, workspace } from "vscode";
...
import {
	type FunctionTemplateBase,
	type IFunctionTemplate,
	TemplateCategory,
} from "./IFunctionTemplate";
...
export class CentralTemplateProvider implements Disposable {
	...
	public async getFunctionTemplates(
		...
		const templates: ITemplates = await this.getTemplates(
			context,
			projectPath,
			language,
			languageModel,
		...
		switch (templateFilter) {
			...
			default:
				const verifiedTemplateIds = getScriptVerifiedTemplateIds(
					...
					.concat(
						getJavaVerifiedTemplateIds().concat(
	...
	public async clearTemplateCache(
		...
		const providers: TemplateProviderBase[] =
			CentralTemplateProvider.getProviders(
				projectPath,
				language,
				languageModel,
				version,
	...
	public async getBindingTemplates(
		context: IActionContext,
		projectPath: string | undefined,
		language: ProjectLanguage,
		languageModel: number | undefined,
		version: FuncVersion,
	): Promise<IBindingTemplate[]> {
		const templates: ITemplates = await this.getTemplates(
			context,
			projectPath,
			language,
	...
	public async tryGetSampleData(
		context: IActionContext,
		version: FuncVersion,
		triggerBindingType: string,
	): Promise<string | undefined> {
		try {
			const templates: IScriptFunctionTemplate[] = <
				IScriptFunctionTemplate[]
			>await this.getFunctionTemplates(
				context,
				undefined,
	...
	public async getProjectTemplateKey(
		context: IActionContext,
		projectPath: string | undefined,
		language: ProjectLanguage,
		languageModel: number | undefined,
	...
	private tryGetCachedProviders(
		projectPath: string | undefined,
		language: ProjectLanguage,
		languageModel: number | undefined,
		version: FuncVersion,
	): CachedProviders | undefined {
		const key: string = this.getCachedProvidersKey(
			language,
			languageModel,
			version,
		);
	...
	private setCachedProviders(
		...
		let key: string = this.getCachedProvidersKey(
			language,
			languageModel,
			version,
		);
	...
	private async getCachedProviders(
		context: IActionContext,
		projectPath: string | undefined,
		language: ProjectLanguage,
		languageModel: number | undefined,
		...
		let cachedProviders = this.tryGetCachedProviders(
			projectPath,
			language,
			languageModel,
			version,
		...
		if (cachedProviders) {
			await Promise.all(
				cachedProviders.providers.map(async (p) => {
					if (
						await p.updateProjKeyIfChanged(
							context,
							projectTemplateKey,
						)
					) {
	...
	private async getTemplates(
		...
		if (templatesTask) {
			...
			templatesTask = this.refreshTemplates(
				context,
		...
		return (
			await Promise.all(
				providers.map(async (provider) => {
					return await this.refreshTemplatesForProvider(
						context,
						provider,
					);
				}),
			...
			return {
				functionTemplates: t1.functionTemplates.concat(
					t2.functionTemplates,
				),
				bindingTemplates: t1.bindingTemplates.concat(
					t2.bindingTemplates,
				),
			};
		});
	...
	private async refreshTemplatesForProvider(
		context: IActionContext,
		...
		try {
				...
				result = await Promise.race([
					...
					delay(timeout).then(() => {
						throw new Error(
							localize(
								"templatesTimeout",
								'Retrieving templates timed out. Modify setting "{0}.{1}" if you want to extend the timeout.',
								ext.prefix,
								requestUtils.timeoutKey,
							),
						);
					}),
				]);
			...
			latestErrorMessage = localize(
				"latestTemplatesError",
		...
		if (result) {
			return {
				functionTemplates: result.functionTemplates.filter((f) =>
					this.includeTemplate(provider, f),
				),
				bindingTemplates: result.bindingTemplates.filter((b) =>
					this.includeTemplate(provider, b),
				),
			};
			...
			throw new Error(
				localize(
					"templateSourceError",
					'Internal error: Failed to get templates for source "{0}".',
					this.templateSource,
				),
			);
		}
	...
	private includeTemplate(
		provider: TemplateProviderBase,
		template: IBindingTemplate | FunctionTemplateBase,
	): boolean {
		return (
			provider.includeTemplate(template) &&
			(!("language" in template) ||
				(template.language.toLowerCase() ===
					provider.language.toLowerCase() &&
					provider.templateSchemaVersion ===
	...
	private async getLatestTemplates(
		...
		if (
			!this.templateSource ||
			this.templateSource === TemplateSource.Latest ||
	...
	private async tryGetCachedTemplates(
		context: IActionContext,
		provider: TemplateProviderBase,
	): Promise<ITemplates | undefined> {
			...
			try {
				...
				ext.outputChannel.appendLog(
					localize(
						...
						"Failed to get cached templates: {0}",
						errorMessage,
					),
				);
	...
	private async tryGetBackupTemplates(
		context: IActionContext,
		provider: TemplateProviderBase,
	): Promise<ITemplates | undefined> {
		if (
			!this.templateSource ||
			this.templateSource === TemplateSource.Backup
		) {
			try {
				...
				ext.outputChannel.appendLog(
					localize(
						...
						"Failed to get backup templates: {0}",
						errorMessage,
					),
				);
	...
	private onConfigChanged(e: ConfigurationChangeEvent): void {
		if (
			e.affectsConfiguration(`${ext.prefix}.${projectTemplateKeySetting}`)
		) {
