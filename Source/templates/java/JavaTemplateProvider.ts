...
export class JavaTemplateProvider extends ScriptTemplateProvider {
	...
	public async getLatestTemplateVersion(): Promise<string> {
		...
		if (match) {
			...
			throw new Error(
				localize(
					"failedToDetectPluginVersion",
					"Failed to detect Azure Functions maven plugin version.",
				),
			);
		}
	}
	...
	public async getLatestTemplates(
		...
		if (regExpResult && regExpResult.length > 3) {
			...
			throw new Error(
				localize(
					"oldFunctionPlugin",
					"You must update the Azure Functions maven plugin for this functionality.",
				),
			);
		}
	}
	...
	private getProjectPath(): string {
		if (this.projectPath) {
			...
			throw new Error(
				localize(
					"projectMustBeOpen",
					"You must have a project open to list Java templates.",
				),
			);
		}
	}
