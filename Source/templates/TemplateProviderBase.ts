...
export abstract class TemplateProviderBase implements Disposable {
	...
	protected async updateCachedValue(
		key: string,
		value: unknown,
	): Promise<void> {
		await ext.context.globalState.update(
			await this.getCacheKey(key),
			value,
		);
	}
	...
	public includeTemplate(
		_template: FunctionTemplateBase | IBindingTemplate,
	): boolean {
		return true;
	}
	...
	public async getBackupTemplateVersion(): Promise<string> {
		return (await AzExtFsExtra.readFile(await this.getBackupVersionPath()))
			.toString()
			.trim();
	}
	...
	protected getBackupPath(): string {
		return ext.context.asAbsolutePath(
			path.join("resources", "backupTemplates", this.backupSubpath),
		);
	}
	 ...
	 */
	public async getProjKey(
		context: IActionContext & Partial<IProjectWizardContext>,
	): Promise<string> {
	 ...
	 */
	public async updateProjKeyIfChanged(
		context: IActionContext,
	...
	public async doesCachedProjKeyMatch(
		context: IActionContext,
	): Promise<boolean> {
		if (this.refreshProjKey) {
