...
export class EventHubListStep extends AzureWizardPromptStep<IEventHubWizardContext> {
	public async prompt(context: IEventHubWizardContext): Promise<void> {
		...
		const picks: IAzureQuickPickItem<Eventhub | undefined>[] = [
			{
				label: localize(
					"newEventHubsNamespace",
					"$(plus) Create new event hub",
				),
				description: "",
				data: undefined,
			},
		...
		const result: Eventhub | undefined = (
	...
	public async getSubWizard(
		context: IEventHubWizardContext,
	): Promise<IWizardOptions<IEventHubWizardContext> | undefined> {
