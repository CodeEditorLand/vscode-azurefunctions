...
export abstract class AzureConnectionCreateStepBase<
	T extends IBindingWizardContext,
> extends AzureWizardExecuteStep<T> {
	...
	public async execute(
		...
		progress.report({
			message: localize("retrieving", "Retrieving connection string..."),
		});

		const result: IConnection = await this.getConnection(context);
		let appSettingKey: string = `${result.name}_${nonNullProp(
			this._setting,
			"resourceType"
		).toUpperCase()}`;
