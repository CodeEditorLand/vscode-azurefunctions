...
import {
	type FunctionV2WizardContext,
	type IFunctionWizardContext,
	getBindingSetting,
} from "../../createFunction/IFunctionWizardContext";
...
export class LocalAppSettingListStep extends BindingSettingStepBase {
			...
			let picks: IAzureQuickPickItem<string | undefined>[] = [
				{
					label: localize(
						"newAppSetting",
						"$(plus) Create new local app setting",
					),
					data: undefined,
				},
			];
			picks = picks.concat(
				existingSettings.map((s: [string, string]) => {
					return {
						data: s[0],
						label: s[0],
						description: this._showHiddenValues ? s[1] : "******",
					};
				}),
			);
			if (picks.length > 1) {
				...
				picks.push(
					this._showHiddenValues
						? hideHiddenValuesItem
	...
	public async getSubWizard(
		...
		| IWizardOptions<IFunctionWizardContext & FunctionV2WizardContext>
		| undefined
	> {
		if (getBindingSetting(context, this._setting)) {
			return undefined;
		} else {
			const azurePromptSteps: AzureWizardPromptStep<
				...
				default:
					...
					return {
						promptSteps: [
							...
							new LocalAppSettingValueStep(valueKey),
						],
						executeSteps: [
							new LocalAppSettingCreateStep(
								this._setting,
								valueKey,
							),
						],
