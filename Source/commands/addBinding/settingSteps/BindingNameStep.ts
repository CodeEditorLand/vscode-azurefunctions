...
export class BindingNameStep extends StringPromptStep {
	...
	public async validateInput(
		...
		if (!val) {
			return localize(
				...
				"The binding name cannot be empty.",
			);
		} else if (await this.bindingExists(context, val)) {
			return localize(
				"existingBindingError",
				'A binding with the name "{0}" already exists.',
				val,
			);
		} else {
	...
	private async bindingExists(
		context: IBindingWizardContext,
		val: string,
	): Promise<boolean> {
		try {
			if (!this._functionJson) {
				this._functionJson = new ParsedFunctionJson(
					await AzExtFsExtra.readJSON(context.functionJsonPath),
				);
			}
