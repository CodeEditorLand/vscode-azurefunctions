...
export class ServiceBusConnectionCreateStep extends AzureConnectionCreateStepBase<
	IServiceBusWizardContext & IBindingWizardContext
> {
	public async getConnection(
		...
		const authRule: SBAuthorizationRule | undefined = authRules.find(
			(ar) =>
				ar.rights && ar.rights.some((r) => r.toLowerCase() === "listen")
		);
		if (!authRule) {
			throw new Error(
				localize(
					"noAuthRule",
					'Failed to get connection string for service bus namespace "{0}".',
					name
				)
			);
		}
