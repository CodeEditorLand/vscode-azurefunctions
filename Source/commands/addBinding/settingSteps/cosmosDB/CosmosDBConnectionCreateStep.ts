...
export class CosmosDBConnectionCreateStep extends AzureConnectionCreateStepBase<
	IFunctionWizardContext & ICosmosDBWizardContext
> {
	public async getConnection(
		context: ICosmosDBWizardContext,
	): Promise<IConnection> {
		const databaseAccount: DatabaseAccountGetResults = nonNullProp(
			context,
			"databaseAccount",
		);
		...
		return {
			name: name,
			connectionString: `AccountEndpoint=${nonNullProp(
				databaseAccount,
				"documentEndpoint",
			)};AccountKey=${nonNullProp(keys, "primaryMasterKey")};`,
		};
	}
