...
export async function enableFunction(
	context: IActionContext,
	node?: FunctionTreeItemBase,
): Promise<void> {
...
async function updateDisabledState(
	context: IActionContext,
	node: FunctionTreeItemBase | undefined,
	isDisabled: boolean,
): Promise<void> {
	if (!node) {
	...
	if (version === FuncVersion.v1) {
		throw new Error(
			localize(
				"notSupportedV1",
				"This operation is not supported for Azure Functions v1.",
			),
		);
	} else {
		await node.parent.parent.setApplicationSetting(
			context,
			node.disabledStateKey,
			String(isDisabled),
		);
	}
