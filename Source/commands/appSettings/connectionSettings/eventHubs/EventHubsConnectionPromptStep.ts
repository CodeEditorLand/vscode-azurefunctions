...
> extends AzureWizardPromptStep<T> {
	...
	public async prompt(context: T): Promise<void> {
		const connectEventNamespaceButton: MessageItem = {
			title: localize(
				"connectEventHubsNamespace",
		...
		const message: string = localize(
			"selectEventHubsNamespace",
			"In order to proceed, you must connect an event hubs namespace for internal use by the Azure Functions runtime.",
		);

		const buttons: MessageItem[] = [
			connectEventNamespaceButton,
			useEmulatorButton,
		];

		const eventHubConnection: string | undefined =
		...
		context.telemetry.properties.eventHubConnectionType =
			context.eventHubsConnectionType;
	}

	public async configureBeforePrompt(context: T): Promise<void> {
		if (
			this.options?.preselectedConnectionType === ConnectionType.Azure ||
			this.options?.preselectedConnectionType === ConnectionType.Emulator
		) {
			context.eventHubsConnectionType =
