...
export async function getStorageConnectionString(
	context: IStorageAccountWizardContext & { useStorageEmulator?: boolean },
	...
	let endpointSuffix: string = nonNullProp(
		context.environment,
...
export async function getEventHubsConnectionString(
	context: IEventHubsConnectionWizardContext & ISubscriptionContext,
): Promise<IResourceResult> {
	...
	const authRulesIterable = client.namespaces.listAuthorizationRules(
		rgName,
		namespaceName,
	);
	...
	if (!manageAccessRules.length) {
		...
		authRuleName = (
			await context.ui.showQuickPick(
				manageAccessRules.map((authRule) => {
					return {
						label: nonNullProp(authRule, "name"),
						suppressPersistence: true,
						description:
							authRule.name === rootKeyName
								? defaultDescription
								: "",
					};
				}),
	...
	const accessKeys: AccessKeys = await client.namespaces.listKeys(
		rgName,
		namespaceName,
		authRuleName,
	);
...
export async function getSqlDatabaseConnectionString(
	...
	if (!username) {
		throw new Error(
			localize(
				"unableToDetermineSqlConnection",
				"Unable to locate SQL server's admin user. Add these credentials to your resource to proceed.",
			),
		);
	}
	...
	if (!password) {
		password = (
			await context.ui.showInputBox({
				prompt: localize(
					"sqlPasswordPrompt",
					"Please enter your SQL server's admin password.",
				),
				password: true,
			})
		).trim();
