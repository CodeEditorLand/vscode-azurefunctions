...
export async function uploadAppSettings(
	...
	await node.runWithTemporaryDescription(
		...
		async () => {
			await uploadAppSettingsInternal(
				context,
				client,
				workspaceFolder,
				exclude
...
export async function uploadAppSettingsInternal(
	...
	if (localSettings.Values) {
		...
		if (
			localSettings.Values[ConnectionKey.Storage] ===
			localStorageEmulatorConnectionString
		) {
				...
				if (
					exclude.some((exclusion) =>
						typeof exclusion === "string"
							...
							: settingName.match(new RegExp(exclusion, "i"))
					)
				) {
		...
		const uploadSettings: string = localize(
			"uploadingSettings",
			"Uploading settings..."
		);
		...
		if (excludedAppSettings.length) {
			ext.outputChannel.appendLog(
				localize("excludedSettings", "Excluded the following settings:")
			);
		...
		ext.outputChannel.appendLog(
			localize("uploadedSettings", "Successfully uploaded settings."),
			{ resourceName: client.fullName }
		);
		// don't wait
		void vscode.window
			.showInformationMessage(
				localize(
					"uploadedSettingsTo",
					'Successfully uploaded settings to "{0}".',
					client.fullName
				),
				viewOutput
			)
