...
import {
	...
	type ILocalSettingsJson,
} from "../../funcConfig/local.settings";
import { dotnetUtils } from "../../utils/dotnetUtils";
import {
	hasNodeJsDependency,
	tryGetPackageJson,
} from "../../utils/nodeJsUtils";
...
export async function detectProjectLanguage(
	...
	try {
		let detectedLangs: ProjectLanguage[] = await detectScriptLanguages(
			context,
			projectPath,
		);
		...
		await detectLanguageFromLocalSettings(
			context,
			detectedLangs,
			projectPath,
		);
...
async function isJavaProject(projectPath: string): Promise<boolean> {
	return (
		(await isMavenProject(projectPath)) ||
		(await isGradleProject(projectPath))
	);
}

export async function isMavenProject(projectPath: string): Promise<boolean> {
	return await AzExtFsExtra.pathExists(
		path.join(projectPath, pomXmlFileName),
	);
}

export async function isGradleProject(projectPath: string): Promise<boolean> {
	return await AzExtFsExtra.pathExists(
		path.join(projectPath, buildGradleFileName),
	);
}

async function isCSharpProject(
	context: IActionContext,
	projectPath: string,
): Promise<boolean> {
	return (
...
async function isFSharpProject(
	context: IActionContext,
	projectPath: string,
): Promise<boolean> {
	return (
...
async function isNodeJsProject(projectPath: string): Promise<boolean> {
	return await AzExtFsExtra.pathExists(
		path.join(projectPath, packageJsonFileName),
	);
}
...
export async function isBallerinaProject(
	projectPath: string,
): Promise<boolean> {
	return await AzExtFsExtra.pathExists(
 ...
 */
async function detectLanguageFromLocalSettings(
	context: IActionContext,
	detectedLangs: ProjectLanguage[],
	projectPath: string,
): Promise<void> {
	try {
 ...
 */
async function detectScriptLanguages(
	context: IActionContext,
	projectPath: string,
): Promise<ProjectLanguage[]> {
	return await telemetryUtils.runWithDurationTelemetry(
		context,
		"detectScriptLangs",
		async () => {
...
export async function detectProjectLanguageModel(
