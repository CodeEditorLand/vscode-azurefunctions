...
import { type DebugConfiguration, type TaskDefinition, window } from "vscode";
import {
	JavaBuildTool,
...
export class JavaInitVSCodeStep extends InitVSCodeStepBase {
	...
	protected async executeCore(
		...
		if (functionAppName) {
			this._debugSubpath = getJavaDebugSubpath(
				functionAppName,
				this._buildTool,
			);
		} else {
			...
			void window.showWarningMessage(
				localize(
					...
					"Cannot parse the Azure Functions name, you may need to specify it in the tasks.json.",
				),
			);
		}
...
export async function getFunctionAppName(
	projectPath: string,
	buildTool: JavaBuildTool | undefined,
	...
	switch (buildTool) {
		...
		default:
			throw new Error(
				localize(
					"invalidJavaBuildTool",
					"Invalid java build tool {0}.",
					buildTool,
				),
			);
	}
...
export function getJavaDebugSubpath(
