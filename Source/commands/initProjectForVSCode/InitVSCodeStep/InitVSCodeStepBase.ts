...
export abstract class InitVSCodeStepBase extends AzureWizardExecuteStep<IProjectWizardContext> {
	...
	public async execute(context: IProjectWizardContext): Promise<void> {
		...
		context.telemetry.properties.isProjectInSubDir = String(
			isSubpath(context.workspacePath, context.projectPath)
		);
		...
		const gitignorePath: string = path.join(
	...
	protected addSubDir(
		...
		const subDir: string = path.relative(
			context.workspacePath,
			context.projectPath
		);
		...
		if (context.workspaceFolder && !isMultiRootWorkspace()) {
			...
			await updateTasks(
				...
				await this.insertNewTasks(
					context,
					getTasks(context.workspaceFolder),
					newTasks
				)
			);
		} else {
	...
	private async insertNewTasks(
		...
		newTasks = newTasks.filter((t1) => {
			...
			return !existingTasks?.some((t2) => {
				...
				return (
					t1String ===
					stringifySorted({ ...t2, label: this.getTaskLabel(t2) })
				);
			});
		});
		...
		for (const existingTask of existingTasks) {
			...
			if (
				...
				newTasks.some(
					(newTask) =>
						existingLabel === this.getTaskLabel(newTask) &&
			...
			if (folder && !isMultiRootWorkspace()) {
				...
				await updateDebugConfigs(
					...
					this.insertLaunchConfig(
						getDebugConfigs(folder),
						newDebugConfig
					)
				);
			} else {
				// otherwise manually edit json
				const launchJsonPath: string = path.join(
					vscodePath,
					launchFileName
				);
				await confirmEditJsonFile(
	...
	private insertLaunchConfig(
		existingConfigs: DebugConfiguration[] | undefined,
	...
	private async writeSettingsJson(
		...
		if (languageModel) {
			settings.push({
				key: projectLanguageModelSetting,
				value: languageModel,
			});
		}
		...
		if (
			path.posix
				.relative(context.projectPath, context.workspacePath)
				.startsWith("../..")
		) {
			settings.push({
		...
		if (this.preDeployTask) {
			settings.push({
				key: preDeployTaskSetting,
				value: this.preDeployTask,
			});
		}
		...
		if (context.workspaceFolder) {
			...
			// otherwise manually edit json
			const settingsJsonPath: string = path.join(
				vscodePath,
				settingsFileName
			);
			await confirmEditJsonFile(
				...
				(data: {}): {} => {
					for (const setting of settings) {
						const key: string = `${setting.prefix || ext.prefix}.${
							setting.key
						}`;
	...
	private async writeExtensionsJson(
		context: IActionContext,
		vscodePath: string,
		language: ProjectLanguage
	): Promise<void> {
		const extensionsJsonPath: string = path.join(
			vscodePath,
			"extensions.json"
		);
		await confirmEditJsonFile(
