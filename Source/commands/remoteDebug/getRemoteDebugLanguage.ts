...
export function getRemoteDebugLanguage(
	...
	if (isNotConsumption) {
		if (
			...
			/(DOCKER\|mcr\.microsoft\.com\/azure-functions\/node)|(Node\|)/i.test(
				siteConfig.linuxFxVersion,
			)
		) {
	...
	throw new Error(
		"Azure Remote Debugging is currently only supported for Node.js Function Apps running on Linux App Service plans. Consumption plans are not supported.",
	);
}
