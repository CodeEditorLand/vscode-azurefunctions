...
import type { IHostJsonV1, IHostJsonV2 } from "../../../funcConfig/host";
...
export class ScriptProjectCreateStep extends ProjectCreateStepBase {
	...
	public async executeCore(
		...
		const localSettingsJsonPath: string = path.join(
			context.projectPath,
			localSettingsFileName,
		);
		if (await confirmOverwriteFile(context, localSettingsJsonPath)) {
			...
			if (
				...
				// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
				this.localSettingsJson.Values!["AzureWebJobsFeatureFlags"] =
					"EnableWorkerIndexing";
			}

			await AzExtFsExtra.writeJSON(
				localSettingsJsonPath,
				this.localSettingsJson,
			);
		}
...
__azurite_db*__.json`),
		...
		const funcIgnorePath: string = path.join(
			context.projectPath,
			".funcignore",
		);
		if (await confirmOverwriteFile(context, funcIgnorePath)) {
			await AzExtFsExtra.writeFile(
				funcIgnorePath,
				this.funcignore.sort().join(os.EOL),
			);
		}
	}
	...
	protected async getHostContent(
		...
		if (
			...
			bundleFeedUtils.overwriteExtensionBundleVersion(
				hostJson,
				"[3.*, 4.0.0)",
				"[4.*, 5.0.0)",
			);
