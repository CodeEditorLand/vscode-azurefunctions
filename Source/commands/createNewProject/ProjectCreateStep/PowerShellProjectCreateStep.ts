...
export class PowerShellProjectCreateStep extends ScriptProjectCreateStep {
	...
	private readonly azModuleGalleryUrl: string =
		`https://aka.ms/PwshPackageInfo?id='${this.azModuleName}'`;
	...
	public async executeCore(
		...
		const profileps1Path: string = path.join(
			context.projectPath,
			profileps1FileName,
		);
		...
		if (majorVersion !== undefined) {
			progress.report({
				message: localize(
					...
					"Successfully retrieved {0} information from PowerShell Gallery",
					this.azModuleName,
				),
			});
		} else {
			progress.report({
				message: localize(
					"failedToConnect",
					"Failed to get {0} module version. Edit the requirements.psd1 file when the powershellgallery.com is accessible.",
					this.azModuleName,
				),
			});
		}

		const requirementspsd1Path: string = path.join(
			context.projectPath,
		...
		if (await confirmOverwriteFile(context, requirementspsd1Path)) {
			if (majorVersion !== undefined) {
				await AzExtFsExtra.writeFile(
					requirementspsd1Path,
					requirementspsd1Online(majorVersion),
				);
			} else {
				await AzExtFsExtra.writeFile(
					requirementspsd1Path,
					requirementspsd1Offine,
				);
			}
		}
	}

	protected async getHostContent(
		context: IProjectWizardContext,
	): Promise<IHostJsonV2> {
	...
	private async tryGetLatestAzModuleMajorVersion(
		...
		progress.report({
			message: localize(
				"connecting",
				"Connecting to PowerShell Gallery...",
			),
		});
	...
	private parseLatestAzModuleVersion(
		response: AzExtPipelineResponse,
	): string {
		/* eslint-disable */
		...
		if (moduleInfo?.entry && Array.isArray(moduleInfo.entry)) {
			const releasedVersions: string[] = moduleInfo.entry
				.filter(
					(entry) =>
						entry["m:properties"]["d:IsPrerelease"]._ === "false",
				)
		...
		throw new Error(
			`Failed to parse latest Az module version ${response.bodyAsText}`,
		);
	}
}
