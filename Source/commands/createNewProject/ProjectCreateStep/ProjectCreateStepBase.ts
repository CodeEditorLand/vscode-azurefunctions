...
export abstract class ProjectCreateStepBase extends AzureWizardExecuteStep<IProjectWizardContext> {
	...
	public async execute(
		...
		if (
			(await gitUtils.isGitInstalled(context.workspacePath)) &&
			!(await gitUtils.isInsideRepo(context.workspacePath))
		) {
		...
		await callWithTelemetryAndErrorHandling(
			"azureFunctions.createNewProjectStarted",
			(startedContext: IActionContext) => {
