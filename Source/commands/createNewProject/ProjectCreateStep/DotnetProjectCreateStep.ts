...
} from "../../../templates/dotnet/executeDotnetTemplateCommand";
...
export class DotnetProjectCreateStep extends ProjectCreateStepBase {
	...
	public async executeCore(context: IProjectWizardContext): Promise<void> {
		...
		if (context.containerizedProject) {
			const runtime = context.workerRuntime?.capabilities.includes(
				"isolated",
			)
				? "dotnet-isolated"
				: "dotnet";
		...
		const args = [
			...
			identity,
			"--arg:name",
			cpUtils.wrapArgInQuotes(projectName),
		...
		args.push(
			"--arg:Framework",
			cpUtils.wrapArgInQuotes(context.workerRuntime?.targetFramework),
		);
	...
	private async confirmOverwriteExisting(
		...
		for (const fileName of filesToCheck) {
			if (
				await AzExtFsExtra.pathExists(
					path.join(context.projectPath, fileName),
				)
			) {
