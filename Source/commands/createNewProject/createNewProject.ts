...
export async function createNewProjectInternal(
	context: IActionContext,
	...
	const wizard: AzureWizard<IFunctionWizardContext> = new AzureWizard(
		wizardContext,
		{
			...
			promptSteps: [
				new FolderListStep(),
				new NewProjectLanguageStep(
					options.templateId,
					options.functionSettings
				),
				new OpenBehaviorStep(),
			],
			executeSteps: optionalExecuteStep
				? [optionalExecuteStep, new OpenFolderStep()]
				: [new OpenFolderStep()],
		}
	);
	await wizard.prompt();
	...
	void window.showInformationMessage(
		localize("finishedCreating", "Finished creating project.")
	);
}
