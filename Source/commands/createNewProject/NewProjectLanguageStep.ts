...
import {
	nodeDefaultModelVersion,
	nodeLearnMoreLink,
	nodeModels,
	ProjectLanguage,
	pythonDefaultModelVersion,
	pythonLearnMoreLink,
	pythonModels,
} from "../../constants";
...
export class NewProjectLanguageStep extends AzureWizardPromptStep<IProjectWizardContext> {
	...
	public async prompt(context: IProjectWizardContext): Promise<void> {
		...
		let languagePicks: IAzureQuickPickItem<
			{ language: ProjectLanguage; model?: number } | undefined
		>[] = [
			{
				label: ProjectLanguage.JavaScript,
				data: { language: ProjectLanguage.JavaScript },
			},
			{
				label: ProjectLanguage.TypeScript,
				data: { language: ProjectLanguage.TypeScript },
			},
			{
				label: ProjectLanguage.CSharp,
				data: { language: ProjectLanguage.CSharp },
			},
			{
				label: ProjectLanguage.Python,
				data: { language: ProjectLanguage.Python },
			},
			{
				label: ProjectLanguage.Java,
				data: { language: ProjectLanguage.Java },
			},
			{
				label: ProjectLanguage.Ballerina,
				data: { language: ProjectLanguage.Ballerina },
			},
			{
				label: ProjectLanguage.PowerShell,
				data: { language: ProjectLanguage.PowerShell },
			},
			{
				label: localize("customHandler", "Custom Handler"),
				data: { language: ProjectLanguage.Custom },
			},
		];
		...
		languagePicks.push({
			label: localize(
				"viewSamples",
				"$(link-external) View sample projects",
			),
			data: undefined,
		...
		if (context.languageFilter) {
			languagePicks = languagePicks.filter((p) => {
				return (
					p.data !== undefined &&
					context.languageFilter?.test(p.data.language)
				);
			});
		}
	...
	public async getSubWizard(
		...
		const wizardOptions: IWizardOptions<IProjectWizardContext> = {
			promptSteps,
			executeSteps,
		};
