...
import { type FuncVersion, getMajorVersion } from "../../../FuncVersion";
...
export async function getPythonVersion(pyAlias: string): Promise<string> {
	const result: cpUtils.ICommandResult = await cpUtils.tryExecuteCommand(
		undefined /*don't display output*/,
...
export async function getSupportedPythonVersions(
	context: IActionContext,
	funcVersionFromSetting: FuncVersion,
): Promise<string[]> {
	// Cache the task so we're not waiting on this every time
	const localVersionTask: Promise<string | null> =
		getLocalFuncCoreToolsVersion(context, undefined);
	...
	for (const [minFuncVersion, pyVersion] of versionInfo) {
		function showVersionWarning(currentVersion: string): void {
			ext.outputChannel.appendLine(
				localize(
					"outOfDateWarning",
					'WARNING: Python version {0}+ is not supported with version "{1}" of the func CLI. Update to at least "{2}" for support.',
					pyVersion,
					currentVersion,
					minFuncVersion,
				),
		...
		try {
			...
			if (
				semver.satisfies(
					minFuncVersion,
					getMajorVersion(funcVersionFromSetting),
				)
			) {
...
export function isSupportedPythonVersion(
