...
export class PythonAliasListStep extends AzureWizardPromptStep<IPythonVenvWizardContext> {
	...
	): Promise<IWizardOptions<IPythonVenvWizardContext> | undefined> {
...
async function getPicks(
	context: IPythonVenvWizardContext,
): Promise<IAzureQuickPickItem<string | boolean>[]> {
	...
	const globalPythonPathSetting: string | undefined = getGlobalSetting(
		"pythonPath",
		"python",
	);
	...
	for (const alias of aliasesToTry) {
		...
		if (
			isSupportedPythonVersion(supportedVersions, version) &&
	...
	picks.push({
		label: localize(
			"enterAlias",
			"$(keyboard) Manually enter Python interpreter or full path",
		),
		data: true,
	});
	...
	if (!context.suppressSkipVenv) {
		picks.push({
			label: localize(
				"skipVenv",
				"$(circle-slash) Skip virtual environment",
			),
			data: false,
