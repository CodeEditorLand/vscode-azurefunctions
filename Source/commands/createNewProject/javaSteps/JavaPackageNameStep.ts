...
function validateJavaName(name: string): string | undefined {
	if (keywords.find((k) => k === name.toLowerCase())) {
		return localize(
			"JavaNameIsKeywordError",
			'"{0}" is a reserved keyword.',
			name,
		);
	} else if (!/^[a-z_$][a-z\d_$]*$/i.test(name)) {
		return localize(
			"JavaNameNotIdentifierError",
			'"{0}" is invalid. It can only contain letters, digits, "_", and "$", and cannot begin with a digit.',
			name,
		);
	} else {
		return undefined;
