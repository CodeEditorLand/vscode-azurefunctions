...
import { type MessageItem, type WorkspaceFolder } from "vscode";
...
export async function tryGetFunctionProjectRoot(
	...
	promptBehavior: MultiProjectPromptBehavior = "silent",
	...
	return await telemetryUtils.runWithDurationTelemetry(
		...
		async () => {
			...
			if (
				...
				const subpath: string | undefined = getWorkspaceSetting(
				...
				if (subpath) {
					...
					if (await isFunctionProject(folderPath)) {
						...
						const hostJsonUris = await findFiles(
							workspaceFolder,
							`*/${hostFileName}`,
						);
...
async function promptForProjectSubpath(
	context: IActionContext,
	workspacePath: string,
	matchingSubpaths: string[],
	promptBehavior: MultiProjectPromptBehavior,
): Promise<string> {
	...
	await context.ui.showWarningMessage(
		...
		{
			learnMoreLink,
			modal: promptBehavior === "modalPrompt",
			stepName: "multipleProjects",
		},
		setDefault,
	);
 ...
 */
export async function verifyProjectPath(
	context: IActionContext,
	workspaceFolder?: WorkspaceFolder | string,
): Promise<string | undefined> {
