...
export class ProgrammingModelStep extends AzureWizardPromptStep<IProjectWizardContext> {
	...
	public async prompt(context: IProjectWizardContext): Promise<void> {
		...
		const modelsPick: IAzureQuickPickItem<number | undefined>[] =
			this._options.models.slice();
		...
		const learnMoreQp = {
			label: localize(
				"learnMore",
				"$(link-external) Learn more about programming models...",
			),
			description: "",
			data: undefined,
		};
	...
	public shouldPrompt(context: IProjectWizardContext): boolean {
		...
		return (
			context.languageModel === undefined &&
