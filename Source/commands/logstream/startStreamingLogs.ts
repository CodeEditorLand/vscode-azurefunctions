...
export async function startStreamingLogs(
	...
	treeItem?: SlotTreeItem | RemoteFunctionTreeItem,
): Promise<void> {
	if (!treeItem) {
	...
	if (site.isLinux) {
		...
		await appservice.startStreamingLogs(
			context,
			site,
			verifyLoggingEnabled,
			treeItem.logStreamLabel,
			treeItem.logStreamPath,
		);
	}
}
 ...
 */
async function openLiveMetricsStream(
	context: IActionContext,
	site: ParsedSite,
	node: AzExtTreeItem,
): Promise<void> {
	...
	if (aiKey) {
		...
		const component: ApplicationInsightsComponent | undefined =
			components.find((c) => c.instrumentationKey === aiKey);
		if (component) {
			...
			throw new Error(
				localize(
					"failedToFindAI",
					"Failed to find application insights component.",
				),
			);
		}
	} else {
...
function isApplicationLoggingEnabled(config: SiteLogsConfig): boolean {
	if (config.applicationLogs) {
		if (config.applicationLogs.fileSystem) {
			return (
				config.applicationLogs.fileSystem.level !== undefined &&
				config.applicationLogs.fileSystem.level.toLowerCase() !== "off"
			);
		} else if (config.applicationLogs.azureBlobStorage) {
			return (
				config.applicationLogs.azureBlobStorage.level !== undefined &&
				config.applicationLogs.azureBlobStorage.level.toLowerCase() !==
					"off"
			);
		} else if (config.applicationLogs.azureTableStorage) {
			return (
				config.applicationLogs.azureTableStorage.level !== undefined &&
				config.applicationLogs.azureTableStorage.level.toLowerCase() !==
					"off"
			);
		}
	}
