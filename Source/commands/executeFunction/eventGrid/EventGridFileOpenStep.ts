...
export class EventGridFileOpenStep extends AzureWizardExecuteStep<EventGridExecuteFunctionContext> {
	...
	public async execute(
		...
		const downloadingMsg: string = localize(
			"downloadingSample",
		...
		const openingFileMsg: string = localize(
			"openingFile",
			"Opening file...",
		);
			...
			modifiedListenerDisposable =
				vscode.workspace.onDidChangeTextDocument(async (event) => {
					if (
						...
						event.document.fileName === document.fileName
					) {
						try {
							await callWithTelemetryAndErrorHandling(
								"eventGridSampleModified",
								async (actionContext: IActionContext) => {
									actionContext.telemetry.properties.eventGridSampleModified =
										"true";
								},
							);
							resolve();
		...
		void new Promise<void>((resolve, reject) => {
			const closedListenerDisposable =
...
async function createTempSampleFile(
	eventSource: string,
	fileName: string,
	contents: {},
): Promise<string> {
