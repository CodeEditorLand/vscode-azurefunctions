...
export async function executeEventGridFunction(
	context: IActionContext,
	_node: FunctionTreeItemBase | IFunction,
): Promise<void> {
	const title: string = localize(
		"executeEGFunction",
		"Execute Event Grid Function",
	);

	const promptSteps: AzureWizardPromptStep<EventGridExecuteFunctionContext>[] =
		[new EventGridSourceStep(), new EventGridTypeStep()];
	...
	const wizard: AzureWizard<EventGridExecuteFunctionContext> =
