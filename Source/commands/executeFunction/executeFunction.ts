...
export async function executeFunctionWithInput(
	context: IActionContext,
	...
	const execute = async () => {
		...
		const client: SiteClient | undefined =
		...
		try {
			...
			if (!client && errorType === "ECONNREFUSED") {
				...
				throw new Error(
					localize(
						"failedToConnect",
						"Failed to connect. Make sure your project is [running locally](https://aka.ms/AA76v2d).",
					),
				);
	...
	const message: string = responseText
		? localize(
				"executedWithResponse",
				'Executed function "{0}". Response: "{1}"',
				func.name,
				responseText,
			)
