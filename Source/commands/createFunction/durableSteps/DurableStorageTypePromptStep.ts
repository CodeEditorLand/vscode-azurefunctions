...
export class DurableStorageTypePromptStep<
...
> extends AzureWizardPromptStep<T> {
	...
	public constructor(functionSettings?: {
		...
		const picks: IAzureQuickPickItem<DurableBackendValues | undefined>[] = [
			{
				...
				data: DurableBackend.Storage,
				suppressPersistence: true,
			},
			{
				label: durableStorageLabels[1],
				data: DurableBackend.Netherite,
				suppressPersistence: true,
			},
			{
				label: durableStorageLabels[2],
				data: DurableBackend.SQL,
				suppressPersistence: true,
			},
			{
				label: durableStorageInfo,
				data: undefined,
				suppressPersistence: true,
			},
		];
