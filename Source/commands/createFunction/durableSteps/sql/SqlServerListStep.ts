...
export class SqlServerListStep<
...
> extends AzureWizardPromptStep<T> {
	...
	public async getSubWizard(
		...
		if (context.sqlServer) {
			...
			promptSteps.push(
				new SqlServerNameStep(),
				new SqlServerUsernameAuthStep(),
				new SqlServerPasswordAuthStep(),
				new ConfirmPreviousInputStep("newSqlAdminPassword", {
					isPassword: true,
				}),
	...
	private async getQuickPicks(
		serverTask: Promise<Server[]>,
	): Promise<IAzureQuickPickItem<Server | undefined>[]> {
		const picks: IAzureQuickPickItem<Server | undefined>[] = [
			{
				label: localize(
					"newSqlServer",
					"$(plus) Create new SQL server",
				),
				description: "",
				data: undefined,
			},
