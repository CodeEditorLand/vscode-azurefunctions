...
> extends AzureWizardExecuteStep<T> {
	...
	public async execute(
		...
		const serverOptions: Server = {
			...
			administratorLoginPassword: nonNullProp(
				context,
				"newSqlAdminPassword",
			),
		};

		context.sqlServer = await client.servers.beginCreateOrUpdateAndWait(
			rgName,
			newServerName,
			serverOptions,
		);
