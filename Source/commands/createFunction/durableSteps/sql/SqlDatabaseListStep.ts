...
export class SqlDatabaseListStep<
...
> extends AzureWizardPromptStep<T> {
		...
		const picksTask: Promise<IAzureQuickPickItem<Database | undefined>[]> =
			this.getQuickPicks(
				uiUtils.listAllIterator(
					client.databases.listByServer(rgName, serverName),
				),
			);
	...
	private async getQuickPicks(
		dbTask: Promise<Server[]>,
	): Promise<IAzureQuickPickItem<Database | undefined>[]> {
		const picks: IAzureQuickPickItem<Server | undefined>[] = [
			{
				label: localize(
					"newSqlDatabase",
					"$(plus) Create new SQL database",
				),
				description: "",
				data: undefined,
			},
