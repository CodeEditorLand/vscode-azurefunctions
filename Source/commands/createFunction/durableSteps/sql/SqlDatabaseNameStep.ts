...
export class SqlDatabaseNameStep<
...
> extends AzureWizardPromptStep<T> {
	...
	public async prompt(context: T): Promise<void> {
		...
		context.newSqlDatabaseName = (
			await context.ui.showInputBox({
				prompt: localize(
					"sqlDatabaseNamePrompt",
					"Provide a SQL database name.",
				),
	...
	private validateInput(name: string | undefined): string | undefined {
		...
		if (dbExists) {
			return localize(
				"sqlDatabaseExists",
				'A SQL database with the name "{0}" already exists.',
				name,
			);
		}
