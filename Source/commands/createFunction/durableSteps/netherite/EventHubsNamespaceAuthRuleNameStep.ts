...
export class EventHubsNamespaceAuthRuleNameStep<
...
> extends AzureWizardPromptStep<T> {
	...
	public async prompt(context: T): Promise<void> {
		...
		const authRulesIterator = client.namespaces.listAuthorizationRules(
			rgName,
			namespaceName,
		);
		...
		context.newAuthRuleName = (
			await context.ui.showInputBox({
				prompt: localize(
					"authRuleNamePrompt",
					"Provide an access policy name for the event hubs namespace.",
				),
	...
	private validateInput(name: string | undefined): string | undefined {
		...
		if (!/^[a-zA-Z0-9]+(?:[._-][a-zA-Z0-9]+)*$/.test(name)) {
			return localize(
				...
				`A name must consist of alphanumeric characters, '.', '-', or '_'. It must start and end with an alphanumeric character.`,
			);
		}
		...
		if (!isNameAvailable) {
			return localize(
				"authRuleExists",
				'An access policy with the name "{0}" already exists.',
				name,
			);
		}
