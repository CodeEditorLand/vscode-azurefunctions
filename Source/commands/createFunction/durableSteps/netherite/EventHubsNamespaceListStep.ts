...
> extends AzureWizardPromptStep<T> {
	...
	public async getSubWizard(
		context: T,
	): Promise<IWizardOptions<T & ISubscriptionContext> | undefined> {
	...
	private async getQuickPicks(
		namespaceTask: Promise<EHNamespace[]>,
	): Promise<IAzureQuickPickItem<EHNamespace | undefined>[]> {
		const picks: IAzureQuickPickItem<EHNamespace | undefined>[] = [
			{
				label: localize(
					"newEventHubsNamespace",
					"$(plus) Create event hubs namespace",
				),
				description: "",
				data: undefined,
			},
