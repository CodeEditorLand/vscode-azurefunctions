...
import type { EventHubManagementClient, Eventhub } from "@azure/arm-eventhub";
import {
...
export class NetheriteEventHubNameStep<
...
> extends AzureWizardPromptStep<T> {
	...
	public async prompt(context: T): Promise<void> {
		...
		if (context.eventHubsNamespace) {
			...
			const eventHubIterator = client.eventHubs.listByNamespace(
				rgName,
				ehNamespaceName,
			);
		...
		context.newEventHubName = (
			await context.ui.showInputBox({
				prompt: localize(
					"eventHubNamePrompt",
					"Enter a name for the new event hub.",
				),
	...
	private validateInput(name: string | undefined): string | undefined {
		...
		if (!isNameAvailable) {
			return localize(
				"eventHubExists",
				'An event hub with the name "{0}" already exists.',
				name,
			);
		}
		return undefined;
