...
export class OpenAPICreateStep extends AzureWizardExecuteStep<IFunctionWizardContext> {
	...
	public async execute(
		wizardContext: IFunctionWizardContext &
			IJavaProjectWizardContext &
			IDotnetFunctionWizardContext,
	): Promise<void> {
		const uris: Uri[] = nonNullProp(
			wizardContext,
		...
		switch (wizardContext.language) {
			...
			default:
				throw new Error(
					localize(
						"notSupported",
						"Not a supported language. We currently support C#, Java, Python, and Typescript",
					),
				);
		}
...
async function validateAutorestInstalled(
	context: IActionContext,
): Promise<void> {
	try {
		await cpUtils.executeCommand(
...
async function addAutorestSpecificTypescriptDependencies(
