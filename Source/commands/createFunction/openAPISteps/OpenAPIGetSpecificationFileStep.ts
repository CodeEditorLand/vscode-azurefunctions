...
import { type OpenDialogOptions, Uri, workspace } from "vscode";
...
export class OpenAPIGetSpecificationFileStep extends AzureWizardPromptStep<IFunctionWizardContext> {
	public async prompt(context: IFunctionWizardContext): Promise<void> {
		...
		if (workspace.workspaceFolders) {
			openDialogOptions.defaultUri = Uri.file(
				workspace.workspaceFolders[0].uri.toString(),
			);
		}

		context.openApiSpecificationFile =
			await context.ui.showOpenDialog(openDialogOptions);
	}
