...
export async function createFunctionInternal(
	context: IActionContext,
	...
	const projectPath: string | undefined = await verifyProjectPath(
		context,
	...
	const { language, languageModel, version, templateSchemaVersion } =
		await verifyInitForVSCode(
			context,
			projectPath,
			options.language,
	...
	const projectTemplateKey: string | undefined = getWorkspaceSetting(
		projectTemplateKeySetting,
		projectPath,
	);
