...
import { type MessageItem, type WorkspaceFolder, window } from "vscode";
			...
			async (currentAttempt: number) => {
				...
				const message: string =
					currentAttempt === 1
						...
						: localize(
								"queryingTriggersAttempt",
								"Querying triggers (Attempt {0}/{1})...",
								currentAttempt,
								retries + 1,
							);
...
async function listHttpTriggerUrls(
	context: IActionContext,
	node: SlotTreeItem,
): Promise<void> {
	...
	const anonFunctions: RemoteFunctionTreeItem[] = <RemoteFunctionTreeItem[]>(
		functions.filter(
			(f) =>
				f instanceof RemoteFunctionTreeItem &&
				f.isHttpTrigger &&
				f.isAnonymous,
		)
	);
	...
	if (!hasHttpTriggers) {
		ext.outputChannel.appendLog(
			localize("noHttpTriggers", "No HTTP triggers found."),
			logOptions,
		);
	}
}
