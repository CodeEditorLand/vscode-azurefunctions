...
export async function showCoreToolsWarning(
	context: IActionContext,
	runtimeVersion: FuncVersion,
	siteName: string,
): Promise<void> {
	...
	if (getWorkspaceSetting<boolean>(showCoreToolsWarningKey)) {
		...
		if (coreToolsVersion) {
			const localVersion = nonNullValue(
				...
				"localCoreToolsVersion",
			);

			if (
				localVersion === FuncVersion.v2 ||
				localVersion === FuncVersion.v3
			) {
				await showCoreToolsEOLWarning(context, localVersion);
			} else if (localVersion !== runtimeVersion) {
				await showCoreToolsMismatchWarning(
					context,
					localVersion,
					runtimeVersion,
					siteName,
...
async function showCoreToolsWarningHelper(
	context: IActionContext,
	message: string,
): Promise<MessageItem> {
	const showCoreToolsWarningKey: string = "showCoreToolsWarning";
...
async function showCoreToolsMismatchWarning(
	context: IActionContext,
	localVersion: FuncVersion,
	runtimeVersion: FuncVersion,
	siteName: string,
