...
export type IFuncDeployContext = {
	...
	IDeployContext &
	ISetConnectionSettingContext &
	ExecuteActivityContext;

export async function deployProductionSlot(
	context: IActionContext,
	target?: vscode.Uri | string | SlotTreeItem,
): Promise<void> {
	await deploy(context, target, undefined);
}

export async function deployProductionSlotByFunctionAppId(
	context: IActionContext,
...
export async function deploySlot(
	...
	await deploy(
		...
		target,
		functionAppId,
		new RegExp(ResolvedFunctionAppResource.pickSlotContextValue),
	);
}

async function deploy(
	actionContext: IActionContext,
	arg1: vscode.Uri | string | SlotTreeItem | undefined,
	arg2: string | {} | undefined,
	_expectedContextValue?: string | RegExp,
): Promise<void> {
	const deployPaths: IDeployPaths = await getDeployFsPath(
		actionContext,
		arg1,
	);
	...
	addLocalFuncTelemetry(
		actionContext,
		deployPaths.workspaceFolder.uri.fsPath,
	);
	...
	const context: IFuncDeployContext = Object.assign(
		actionContext,
		deployPaths,
		{
	...
	const node: SlotTreeItem = await getDeployNode(
		context,
		ext.rgApi.tree,
		arg1,
		arg2,
	...
	if (node.contextValue.includes("container")) {
		...
		await context.ui.showWarningMessage(
			localize(
				...
				"Deploy is not currently supported for containerized function apps within the Azure Functions extension. Please read here to learn how to deploy your project.",
			),
			{ learnMoreLink },
		);
	...
	const { language, languageModel, version } = await verifyInitForVSCode(
		context,
	...
	if (!isZipDeploy && node.site.isLinux && isConsumption) {
		ext.outputChannel.appendLog(
			localize(
				"linuxConsZipOnly",
				'WARNING: Using zip deploy because scm type "{0}" is not supported on Linux consumption',
				siteConfig.scmType,
			),
	...
	const doRemoteBuild: boolean | undefined =
	...
	const { shouldValidateEventHubs, shouldValidateSqlDb } =
		await shouldValidateConnections(
			durableStorageType,
			client,
			context.projectPath,
		);
	...
	if (
		(language === ProjectLanguage.CSharp && !node.site.isLinux) ||
		durableStorageType
	) {
		await updateWorkerProcessTo64BitIfRequired(
			context,
			siteConfig,
			node,
			language,
...
async function updateWorkerProcessTo64BitIfRequired(
	...
	const functionProject: string | undefined = await tryGetFunctionProjectRoot(
		context,
		context.workspaceFolder,
	);
	...
	if (platformTarget === "x64" && siteConfig.use32BitWorkerProcess === true) {
		const message: string = localize(
			"overwriteSetting",
			'The remote app targets "{0}", but your local project targets "{1}". Update remote app to "{1}"?',
			"32 bit",
			"64 bit",
		);
		...
		const dialogResult: vscode.MessageItem =
			await context.ui.showWarningMessage(
				message,
				{ modal: true, stepName: "mismatch64bit" },
				DialogResponses.yes,
				deployAnyway,
			);
...
async function validateGlobSettings(
	context: IActionContext,
	fsPath: string,
): Promise<void> {
