...
export class FunctionAppListStep extends AzureWizardPromptStep<IFuncDeployContext> {
	public async prompt(context: IFuncDeployContext): Promise<void> {
		context.site = (
			await context.ui.showQuickPick(this.getPicks(context), {
				placeHolder: vscode.l10n.t("Select a function app"),
			})
		).data;
	}
	...
	): Promise<IAzureQuickPickItem<Site | undefined>[]> {
		...
		const qp: IAzureQuickPickItem<Site | undefined>[] = sites
			...
			.map((fa) => {
				return {
					...
					description: parseAzureResourceGroupId(
						nonNullProp(fa, "id"),
					).resourceGroup,
					data: fa,
				};
	...
	public async getSubWizard(
		context: IFuncDeployContext,
	): Promise<IWizardOptions<IFuncDeployContext> | undefined> {
		if (context.site) {
		...
		return {
			...
			promptSteps:
				promptSteps as unknown as AzureWizardPromptStep<IFuncDeployContext>[],
			executeSteps:
				executeSteps as unknown as AzureWizardExecuteStep<IFuncDeployContext>[],
		};
	}
}
