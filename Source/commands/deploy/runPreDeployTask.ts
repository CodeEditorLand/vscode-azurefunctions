...
export async function runPreDeployTask(
	...
	const preDeployTask: string | undefined = getWorkspaceSetting(
		preDeployTaskSetting,
		deployFsPath,
	);
	...
	let result: IPreDeployTaskResult = await tryRunPreDeployTask(
		context,
		deployFsPath,
		scmType,
	);
	...
	if (result.failedToFindTask) {
		throw new Error(
			localize(
				"noPreDeployTaskError",
				'Failed to find preDeploy task "{0}". Modify the setting "{1}.{2}" or add that task to {3}.',
				result.taskName,
				ext.prefix,
				preDeployTaskSetting,
				tasksFileName,
...
async function promptToBuildNativeDeps(
	context: IDeployContext,
	deployFsPath: string,
	scmType: string | undefined,
