...
export class FunctionAppCreateStep extends AzureWizardExecuteStep<IFunctionAppWizardContext> {
	...
	public async execute(
		context: IFlexFunctionAppWizardContext,
		...
		context.site = context.newFlexSku
			? await this.createFlexFunctionApp(
					context,
					rgName,
					siteName,
					context.newFlexSku,
		...
		if (!site.isLinux) {
			...
			try {
				...
				context.telemetry.properties.fileLoggingError =
		...
		site.properties.functionAppConfig = {
			deployment: {
				storage: {
					type: "blobContainer",
					value: `${
						context.storageAccount?.primaryEndpoints?.blob
					}app-package-${context.newSiteName?.substring(
						0,
						32,
					)}-${randomUtils.getRandomHexString(7)}`,
					authentication: {
			...
			scaleAndConcurrency: {
				maximumInstanceCount:
					context.newFlexMaximumInstanceCount ??
					sku.maximumInstanceCount.defaultValue,
				instanceMemoryMB:
					context.newFlexInstanceMemoryMB ??
					sku.instanceMemoryMB.find((im) => im.isDefault)?.size ??
					2048,
				alwaysReady: [],
				triggers: null,
			},
	...
	private async getNewSiteConfig(
		...
		if (stack) {
			const stackSettings: FunctionAppRuntimeSettings = nonNullProp(
			...
			appSettings = appSettings.concat(
				...
				...Object.entries(stackSettings.appSettingsDictionary).map(
					([name, value]) => {
						return { name, value };
					},
				),
		...
		if (
			...
			!(
				context.language === ProjectLanguage.CSharpScript &&
		...
		if (context.appInsightsComponent) {
			...
			if (
				isElasticPremium &&
				context.newSiteStack?.stack.value === "java"
			) {
	...
	async createFlexFunctionApp(
		...
		const options: AzExtRequestPrepareOptions = {
			...
			method: "PUT",
			body: JSON.stringify(
				await this.getNewFlexSite(context, sku),
			) as unknown as RequestBodyType,
			headers,
		};
		...
		if (result && result.status >= 200 && result.status < 300) {
			const site = result.parsedBody as Site & {
				properties: FlexFunctionAppProperties;
			};
			const storageConnectionString: string = (
				await getStorageConnectionString(context)
			).connectionString;
		...
		throw new Error(
			parseError(result.parsedBody).message ||
				localize(
					"failedToCreateFlexFunctionApp",
					'Failed to create flex function app "{0}".',
					siteName,
				),
		);
	}
...
function getNewFileShareName(siteName: string): string {
	...
	return (
		...
		getRandomHexString(randomLetters)
	);
}
...
async function tryCreateStorageContainer(
	site: Site & { properties: FlexFunctionAppProperties },
