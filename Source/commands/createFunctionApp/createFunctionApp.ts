...
import {
	type ICreateFunctionAppContext,
	SubscriptionTreeItem,
} from "../../tree/SubscriptionTreeItem";
...
export async function createFunctionApp(
	context: IActionContext & Partial<ICreateFunctionAppContext>,
	...
	if (typeof subscription === "string") {
		...
		node =
			await ext.rgApi.appResourceTree.showTreeItemPicker<AzExtParentTreeItem>(
	...
	const funcAppNode: SlotTreeItem | ContainerTreeItem =
		await SubscriptionTreeItem.createChild(
			context as ICreateFunctionAppContext,
...
export async function createFunctionAppAdvanced(
	context: IActionContext,
	subscription?: AzExtParentTreeItem | string,
	nodesOrNewResourceGroupName?: string | (string | AzExtParentTreeItem)[],
): Promise<string | undefined> {
	return await createFunctionApp(
		{ ...context, advancedCreation: true },
		subscription,
		nodesOrNewResourceGroupName,
	);
}
