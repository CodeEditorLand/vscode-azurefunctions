...
> {
		...
		for (const majorVersion of stack.majorVersions) {
			const minorVersions: AppStackMinorVersion<FunctionAppRuntimes>[] =
				majorVersion.minorVersions.filter((mv) => {
					return (
						(mv.stackSettings.linuxRuntimeSettings &&
							mv.stackSettings.linuxRuntimeSettings.supportedFunctionsExtensionVersions.includes(
								context.version,
							)) ||
						(mv.stackSettings.windowsRuntimeSettings &&
							mv.stackSettings.windowsRuntimeSettings.supportedFunctionsExtensionVersions.includes(
								context.version,
							))
					);
				});
	...
	picks.sort((p1, p2) => {
		...
		return p1.data.stack.value !== p2.data.stack.value
			? 0
			: // keep order as-is if they're different stacks (i.e. Node.js vs. .NET)
	...
	if (hasEndOfLife) {
		picks.push({
			label: localize(
				"endOfLife",
				`$(extensions-warning-message) Some stacks have an end of support deadline coming up. Learn more...`,
			),
			onPicked: async () => {
...
function getFlagOs(
	...
	if (
		[ss.linuxRuntimeSettings, ss.windowsRuntimeSettings].every(
			(s) => !s || s[key],
		)
	) {
		...
		try {
			...
			const result: AzExtPipelineResponse = await client.sendRequest(
				createPipelineRequest({
					method: "GET",
					url: requestUtils.createRequestUrl(
						"/providers/Microsoft.Web/functionappstacks",
						{
							"api-version": "2020-10-01",
							removeDeprecatedStacks: String(
								!getWorkspaceSetting<boolean>(
									"showDeprecatedStacks",
								),
							),
						},
					),
			...
			context.telemetry.properties.getStacksError =
		...
		const getFlexStack = async (stack: string) => {
			const result: AzExtPipelineResponse = await client.sendRequest(
				createPipelineRequest({
					method: "GET",
					url: requestUtils.createRequestUrl(
						`providers/Microsoft.Web/locations/${location.name}/functionAppStacks`,
						{
				...
				stack.properties.majorVersions =
					stack.properties.majorVersions.filter((mv) => {
						mv.minorVersions = mv.minorVersions.filter((minor) => {
							...
							return (
								minor.stackSettings.linuxRuntimeSettings &&
								minor.stackSettings.linuxRuntimeSettings
									?.Sku !== null
							);
						});
...
function removeDeprecatedStacks(stacks: FunctionAppStack[]) {
	...
	const deprecatedDotnetStacks: string[] = [
		"dotnetcore2",
		"dotnetcore3.1",
		"dotnet5",
	];
	for (const stack of stacks) {
		if (stack.value === "dotnet") {
			stack.majorVersions = stack.majorVersions.filter(
				(mv) => !deprecatedDotnetStacks.includes(mv.value),
			);
		}
	}
}
...
export function shouldShowEolWarning(
