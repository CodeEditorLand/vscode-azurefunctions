...
export class FunctionAppStackStep extends AzureWizardPromptStep<IFlexFunctionAppWizardContext> {
	public async prompt(context: IFlexFunctionAppWizardContext): Promise<void> {
			...
			const options: AgentQuickPickOptions = {
				...
				agentMetadata: {
					...
					parameterDisplayDescription:
						"The runtime stack to use for the function app.",
				},
			};
		...
		if (isFlex) {
			context.newFlexSku =
				result.minorVersion.stackSettings.linuxRuntimeSettings?.Sku &&
				result.minorVersion.stackSettings.linuxRuntimeSettings?.Sku[0];
		}
	}
	...
	public async getSubWizard(
		...
		if (context.newFlexSku) {
			promptSteps.push(
				new InstanceMemoryMBPromptStep(),
				new MaximumInstanceCountPromptStep(),
			);
		}
	...
	> {
		...
		if (picks.length === 0) {
			const noPicksMessage = context.stackFilter
				? localize(
						"noStacksFoundWithFilter",
						'$(warning) No stacks found for "{0}" on Azure Functions v{1}',
						context.stackFilter,
						majorVersion,
					)
				: localize(
			...
			picks.push({
				...
				onPicked: async () => {
					await openUrl(
						"https://aka.ms/function-runtime-host-warning",
					);
				},
		...
		picks.push({
			label: localize(
				"changeFuncVersion",
				"$(gear) Change Azure Functions version",
			),
			description:
				localize(
					"currentFuncVersion",
