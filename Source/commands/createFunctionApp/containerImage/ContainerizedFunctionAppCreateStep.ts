...
export class ContainerizedFunctionAppCreateStep extends AzureWizardExecuteStep<IFunctionAppWizardContext> {
	...
	public async execute(
		...
		if (
			!context.deployWorkspaceResult?.registryLoginServer ||
			!context.deployWorkspaceResult?.imageName
		) {
			throw new Error(
				localize(
					"failToCreateApp",
					"Failed to create function app. There was an error creating the necessary container resources.",
				),
			);
		}
		...
		const containerSite = Object.assign(context.site, {
			defaultHostUrl: `https://${context.site.defaultHostName}`,
			fullName: context.site.name,
			isSlot: false,
		});
			...
			siteConfig: {
				...
				appSettings: [
					...
					},
					{
						...
						value: context.deployWorkspaceResult
							?.registryLoginServer,
					},
					{
...
async function pingContainerizedFunctionApp(
	context: IFunctionAppWizardContext,
	...
	const headers = createHttpHeaders({
		"x-functions-key":
			(
				await client.webApps.listHostKeys(
					nonNullProp(site, "resourceGroup"),
					nonNullProp(site, "name"),
				)
			).masterKey || "",
	});
