...
export class ParsedFunctionJson {
	...
	public constructor(data: any) {
		...
		if (
			typeof data === "object" &&
			data !== null &&
			(data.bindings === undefined || data.bindings instanceof Array)
		) {
	...
	public get isHttpTrigger(): boolean {
		return (
			!!this.triggerBinding &&
			!!this.triggerBinding.type &&
			/^http/i.test(this.triggerBinding.type)
		);
	}
	...
	public get isTimerTrigger(): boolean {
		return (
			!!this.triggerBinding &&
			!!this.triggerBinding.type &&
			/^timer/i.test(this.triggerBinding.type)
		);
	}

	public get authLevel(): HttpAuthLevel | undefined {
		if (this.triggerBinding && this.triggerBinding.authLevel) {
			return HttpAuthLevel[
				this.triggerBinding.authLevel.toLowerCase() as HttpAuthLevel
			];
		} else {
			return undefined;
		}
