...
export function getTemplateVersionFromLanguageAndModel(
	language?: ProjectLanguage,
	languageModel?: number,
): TemplateSchemaVersion {
	/* as of now, only Python V2 supports v2 templates */
	return isPythonV2Plus(language, languageModel)
		? TemplateSchemaVersion.v2
		: TemplateSchemaVersion.v1;
}

export function verifyTemplateIsV1(
	template?: FunctionTemplateBase | IBindingTemplate,
): template is IFunctionTemplate {
	return !!template && !("wizards" in template);
}

export function verifyTemplateIsV2(
	template?: FunctionTemplateBase | IBindingTemplate,
): template is FunctionV2Template {
	return !verifyTemplateIsV1(template);
}

export function assertTemplateIsV1(
	template?: FunctionTemplateBase | IBindingTemplate,
): asserts template is IFunctionTemplate {
	if (!verifyTemplateIsV1(template)) {
		throw new Error("Expected template to be v1");
	}
}

export function assertTemplateIsV2(
	template?: FunctionTemplateBase | IBindingTemplate,
): asserts template is FunctionV2Template {
