...
export async function verifyExtensionBundle(
	context: IFunctionWizardContext | IBindingWizardContext,
	...
	if (
		...
		context.language === ProjectLanguage.CSharp ||
	...
	if (
		(await hasExtensionsVSCodeConfig(context, context.workspacePath)) ||
		(await hasExtensionsCsproj(context, context.projectPath))
	) {
		...
		if (!(await AzExtFsExtra.pathExists(hostFilePath))) {
			...
			try {
				...
				context.telemetry.properties.bundleResult =
					"failedToParseHostJson";
...
export async function verifyExtensionsConfig(
	context: IActionContext,
	workspacePath: string,
	projectPath: string,
): Promise<void> {
	if (
		(await hasExtensionsCsproj(context, projectPath)) &&
		!(await hasExtensionsVSCodeConfig(context, workspacePath))
	) {
		...
		await promptToReinitializeProject(
			...
			message,
			"https://aka.ms/AA8wo2c",
			context,
		);
	}
}

async function hasExtensionsVSCodeConfig(
	context: IActionContext,
	workspacePath: string,
): Promise<boolean> {
...
async function hasExtensionsCsproj(
	context: IActionContext,
	projectPath: string,
): Promise<boolean> {
	let result: boolean = false;
