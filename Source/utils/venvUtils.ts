...
export namespace venvUtils {
	...
	export async function runPipInstallCommandIfPossible(
		projectPath: string,
		venvName?: string,
	): Promise<void> {
		...
		if (await AzExtFsExtra.pathExists(requirementsPath)) {
			try {
				...
				ext.outputChannel.appendLog(
					localize(
						"pipInstallFailure",
						'WARNING: Failed to install packages in your virtual environment. Run "pip install" manually instead.',
					),
				);
			}
		}
	...
	export function convertToVenvCommand(
		command: string,
		folderPath: string,
	): string {
	...
	export function convertToVenvPythonCommand(
		command: string,
		venvName: string,
	...
	export async function runCommandInVenv(
		command: string,
		venvName: string,
		folderPath: string,
	): Promise<void> {
	...
	function getVenvActivateCommand(
		venvName: string,
		terminal: Terminal,
		platform: NodeJS.Platform,
	): string {
