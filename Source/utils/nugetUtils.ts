...
export namespace nugetUtils {
	...
	export function tryGetMaxInRange(
		rangeString: string,
		versions: string[],
	): string | undefined {
	...
	function parseVersionRange(value: string): IVersionRange | string {
		const error: Error = new Error(
			localize("invalidRange", 'Invalid range "{0}"', value),
		);
		...
		if (value[0] === "(" || value[0] === "[") {
			...
			if (value.includes(wildcard)) {
				// If the value has wildcards, it denotes a range
				return value;
			} else {
