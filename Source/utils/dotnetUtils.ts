...
export namespace dotnetUtils {
	export const isolatedSdkName: string =
	...
	export class ProjectFile {
		...
		public async getContents(): Promise<string> {
			if (this._cachedContents === undefined) {
				this._cachedContents = await AzExtFsExtra.readFile(
					this.fullPath
				);
			}
	 ...
	 */
	export async function getProjFiles(
	...
	export async function getTargetFramework(
		projFile: ProjectFile
	): Promise<string> {
		return await getPropertyInProjFile(projFile, "TargetFramework");
	}

	export async function tryGetFuncVersion(
		projFile: ProjectFile
	): Promise<string | undefined> {
		try {
			return await getPropertyInProjFile(
				projFile,
	...
	export async function tryGetPlatformTarget(
		projFile: ProjectFile
	): Promise<string | undefined> {
		try {
	...
	export async function getIsIsolated(
		projFile: ProjectFile
	): Promise<boolean> {
		try {
			return (await projFile.getContents())
				.toLowerCase()
				.includes(isolatedSdkName.toLowerCase());
		} catch {
			return false;
		}
	}

	async function getPropertyInProjFile(
		projFile: ProjectFile,
		prop: string
	): Promise<string> {
		...
		if (!matches) {
			throw new Error(
				localize(
					"failedToFindProp",
					'Failed to find "{0}" in project file "{1}".',
					prop,
					projFile.name
				)
			);
	...
	export async function getTemplateKeyFromProjFile(
		...
		if (projectPath && (await AzExtFsExtra.pathExists(projectPath))) {
			const projFiles = await getProjFiles(
				context,
				language,
				projectPath
			);
	...
	export function getTemplateKeyFromFeedEntry(
		runtimeInfo: cliFeedUtils.IWorkerRuntime
	): string {
		const isIsolated =
	...
	function getProjectTemplateKey(
	...
	export function getPackageReferences(
		xml: CSProjXml
	): CSProjPackageReference[] {
