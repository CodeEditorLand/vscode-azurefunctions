 ...
 */
function getPseudononymousStringHash(
	s: string,
...
export function registerStaticContent(
	...
	return registerContentProvider().registerTextDocumentContent(
		content,
		filename,
	);
}

export async function showMarkdownPreviewContent(
	content: string,
	filename: string,
	openToSide?: boolean,
): Promise<void> {
...
export async function showMarkdownPreviewFile(
	uri: vscode.Uri,
	openToSide = false,
): Promise<void> {
	await vscode.commands.executeCommand(
		openToSide ? "markdown.showPreviewToSide" : "markdown.showPreview",
		uri,
	);
}
