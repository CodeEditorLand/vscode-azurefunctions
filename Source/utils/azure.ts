...
export async function promptForResource<T extends IBaseResourceWithName>(
	...
	const picksTask: Promise<IAzureQuickPickItem<T | undefined>[]> =
		resourcesTask.then((resources: T[]) => {
			const picks: IAzureQuickPickItem<T | undefined>[] = !isArray(
				resources,
			)
				? []
			...
			picks.push({
				label: picks.length
					...
					: localize(
							"skippedNoResources",
							"$(warning) Skipped because no matching resource was found",
						),
				data: undefined,
...
export async function registerProviders(
	...
	const providerContext: IAppServiceWizardContext = Object.assign(
		context,
