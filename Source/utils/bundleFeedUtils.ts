...
import type { IBundleMetadata, IHostJsonV2 } from "../funcConfig/host";
...
export namespace bundleFeedUtils {
	...
	export async function getLatestTemplateVersion(
		...
		const validVersions: string[] = Object.keys(
			feed.templates[templateSchemaVersion],
		...
		if (bundleVersion) {
			...
			throw new Error(
				localize(
					"failedToFindBundleVersion",
					'Failed to find bundle version satisfying range "{0}".',
	...
	export async function getReleaseV2(
		context: IActionContext,
		bundleMetadata: IBundleMetadata | undefined,
		templateVersion: string,
	): Promise<ITemplatesReleaseV2> {
		const feed: IBundleFeed = await getBundleFeed(context, bundleMetadata);
		return feed.templates.v2[templateVersion];
	}

	export function isBundleTemplate(
	...
	export async function getLatestVersionRange(
		context: IActionContext,
	): Promise<string> {
	...
	export async function addDefaultBundle(
		context: IActionContext,
	...
	function isTemplateOfType(
		template: Partial<IFunctionTemplate>,
		templateType: string,
	): boolean {
		return !!template.id
			?.toLowerCase()
			.includes(templateType.toLowerCase());
	}

	async function getBundleFeed(
		context: IActionContext,
		bundleMetadata: IBundleMetadata | undefined,
	): Promise<IBundleFeed> {
		const bundleId: string =
			(bundleMetadata && bundleMetadata.id) || defaultBundleId;
	...
	export function overwriteExtensionBundleVersion(
		hostJson: IHostJsonV2,
		...
		if (
			hostJson.extensionBundle &&
			hostJson.extensionBundle.version === expectedRange
		) {
