 ...
 */
export async function getRootWorkspaceFolder(): Promise<
...
export async function findFiles(
	base: vscode.WorkspaceFolder | string,
	pattern: string,
): Promise<vscode.Uri[]> {
...
export async function selectWorkspaceItem(
	...
	return folder && folder.data
		? folder.data
		: (await context.ui.showOpenDialog(options))[0].fsPath;
}

export function getContainingWorkspace(
	fsPath: string,
): vscode.WorkspaceFolder | undefined {
	...
	return openFolders.find((f: vscode.WorkspaceFolder): boolean => {
		return (
			...
			fsUtils.isSubpath(f.uri.fsPath, fsPath)
		);
	});
}
