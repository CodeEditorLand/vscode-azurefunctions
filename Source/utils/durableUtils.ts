...
import {
	type IHostJsonV2,
	type INetheriteTaskJson,
	type ISqlTaskJson,
	type IStorageTaskJson,
} from "../funcConfig/host";
...
export namespace durableUtils {
	...
	export function requiresDurableStorageSetup(
		...
		return (
			...
			templateRequiresDurableStorageSetup(
	...
	export function templateRequiresDurableStorageSetup(
		templateId: string,
		language?: string,
	): boolean {
		...
		return (
			durableOrchestrator.test(templateId) ||
			durableEntity.test(templateId)
		);
	}

	export async function getStorageTypeFromWorkspace(
		language: string | undefined,
		projectPath: string,
	): Promise<DurableBackendValues | undefined> {
		const hasDurableStorage: boolean = await verifyHasDurableStorage(
			language,
			projectPath,
		);
	...
	export async function verifyHasDurableStorage(
		language: string | undefined,
		projectPath: string,
	): Promise<boolean> {
		switch (language) {
	...
	async function nodeProjectHasDurableDependency(
		projectPath: string,
	): Promise<boolean> {
		return await hasNodeJsDependency(projectPath, nodeDfPackage);
	}

	async function dotnetProjectHasDurableDependency(
		projectPath: string,
	): Promise<boolean> {
		...
		if (
			!(
				csProjPaths?.[0]?.path &&
				(await AzExtFsExtra.pathExists(csProjPaths[0].path))
			)
		) {
			return false;
		}

		const csProjContents: string = await AzExtFsExtra.readFile(
			csProjPaths[0].path,
		);
		...
		return new Promise((resolve) => {
			xml2js.parseString(
				csProjContents,
				(err: Error, result: unknown): void => {
					if (result && !err) {
						resolve(
							dotnetUtils
								.getPackageReferences(result)
								.some((p) => /Durable/i.test(p.Include)),
						);
					}
				},
			);
	...
	async function pythonProjectHasDurableDependency(
		projectPath: string,
	): Promise<boolean> {
	...
	export async function tryInstallDurableDependencies(
		context: IFunctionWizardContext,
	): Promise<void> {
	...
	async function installDotnetDependencies(
		context: IFunctionWizardContext,
	): Promise<void> {
		...
		if (failedPackages.length) {
			ext.outputChannel.appendLog(
				localize(
					"durableDependencyInstallFailed",
					'WARNING: Failed to install and update Durable Functions NuGet packages to the root .csproj project file. You may need to install the following packages manually: "{0}".',
					failedPackages.join('", "'),
				),
			);
		}
