...
export class RemoteFunctionTreeItem extends FunctionTreeItemBase {
	...
	public static async create(
		...
		parent: RemoteFunctionsTreeItem,
		func: RemoteFunction
	): Promise<RemoteFunctionTreeItem> {
		const ti: RemoteFunctionTreeItem = new RemoteFunctionTreeItem(
			parent,
			func
		);
	...
	public get logStreamPath(): string {
		return `application/functions/function/${encodeURIComponent(
			this.function.name
		)}`;
	}
