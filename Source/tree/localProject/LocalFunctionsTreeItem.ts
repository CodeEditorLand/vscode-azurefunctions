...
export class LocalFunctionsTreeItem extends FunctionsTreeItemBase {
	...
	public async loadMoreChildrenImpl(
		...
		try {
			const localFunctionsResult = await listLocalFunctions(
				this.parent.project,
			);
			...
			for (const invalidLocalFunction of localFunctionsResult.invalidFunctions) {
				children.push(
					new InvalidTreeItem(this, invalidLocalFunction.error, {
			...
			if (this.parent.preCompiledProjectPath) {
				const ti: GenericTreeItem = new GenericTreeItem(this, {
					label: localize(
						"runBuildTask",
						"Run build task to update this list...",
					),
			...
			if (error instanceof ProjectNotRunningError) {
				const ti: GenericTreeItem = new GenericTreeItem(this, {
					label: localize(
						"startDebugging",
						"Start debugging to update this list...",
					),
					iconPath: new ThemeIcon("info"),
...
export async function getFunctionFolders(
	context: IActionContext,
	projectPath: string,
): Promise<string[]> {
	return await telemetryUtils.runWithDurationTelemetry(
		...
		"getFuncs",
		async () => {
			const funcJsonUris = await findFiles(
				projectPath,
