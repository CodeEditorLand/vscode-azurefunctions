...
export function getProjectContextValue(
	...
	return (
		[contextValuePrefix, source, access, resource, ...parts].join(
...
export function isRemoteProjectCV(contextValue: string | RegExp): boolean {
	return (
		isProjectCV(contextValue) &&
		(matchesAnyPart(contextValue, ProjectSource.Remote) ||
			!matchesAnyPart(contextValue, ProjectSource.Local))
	);
}
...
export function matchesAnyPart(
	contextValue: string | RegExp,
	...parts: string[]
): boolean {
	const data: string = normalizeContextValue(contextValue);
