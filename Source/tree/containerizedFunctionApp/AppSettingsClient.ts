...
import {
	type AppSettingsClientProvider,
...
export class ContainerAppSettingsClientProvider
...
export class ContainerAppSettingsClient implements IAppSettingsClient {
	...
	public async listApplicationSettings(): Promise<StringDictionary> {
		return await this._client.webApps.listApplicationSettings(
			this._resourceGroup,
			this._siteName,
		);
	}
	...
	public async updateApplicationSettings(
		...
		try {
			response = await this._client.webApps.updateApplicationSettings(
				this._resourceGroup,
				this._siteName,
				appSettings,
			);
