...
import type { FunctionEnvelope, Site } from "@azure/arm-appservice";
...
export class ContainerFunctionsTreeItem extends FunctionsTreeItemBase {
	...
	constructor(
		public readonly parent: ContainerTreeItem,
        ...
        */
		...
		const funcs = await retry<FunctionEnvelope[]>(
			async (attempt: number) => {
				...
				const response = await uiUtils.listAllIterator(
					client.webApps.listFunctions(
				...
				if (response.length === 1 && isWarmupFunction(response[0])) {
					...
					context.telemetry.measurements.listFunctionsAttempt =
						attempt;
				}
		...
		return await this.createTreeItemsWithErrorHandling(
			...
			async (fe: FunctionEnvelope) =>
				await ContainerFunctionTreeItem.create(
					context,
					this,
...
function isWarmupFunction(func: FunctionEnvelope): boolean {
	try {
		return (
			!!func.id &&
			getFunctionNameFromId(func.id).toLowerCase() === "warmup"
		);
	} catch {
		return false;
	}
