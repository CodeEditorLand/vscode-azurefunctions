...
import {
	FunctionAppHostingPlanStep,
	setConsumptionPlanProperties,
} from "../commands/createFunctionApp/FunctionAppHostingPlanStep";
...
import {
	getRootFunctionsWorkerRuntime,
	getWorkspaceSetting,
	getWorkspaceSettingFromAnyFolder,
...
export class SubscriptionTreeItem extends SubscriptionTreeItemBase {
	...
	public static async createChild(
		context: ICreateFunctionAppContext,
		subscription: SubscriptionTreeItem,
	): Promise<SlotTreeItem | ContainerTreeItem> {
		...
		if (context.dockerfilePath) {
			...
			const functionAppWizard =
				await createFunctionAppWizard(wizardContext);
		...
		if (!context.advancedCreation) {
			...
			if (!newName) {
				throw new Error(
					localize(
						"noUniqueName",
						"Failed to generate unique name for resources. Use advanced creation to manually enter resource names.",
					),
				);
			}
		...
		if (context.dockerfilePath) {
			const resolved = new ResolvedContainerizedFunctionAppResource(
				subscription.subscription,
...
async function getDefaultFuncVersion(
	context: ICreateFunctionAppContext,
): Promise<FuncVersion> {
...
async function createFunctionAppWizard(
	...
	if (wizardContext.version === FuncVersion.v1) {
