...
} from "../workspace/listLocalProjects";
...
export class AzureAccountTreeItemWithProjects extends AzureAccountTreeItemBase {
	...
	public async loadMoreChildrenImpl(
		...
		for (const project of workspaceProjects.initializedProjects) {
			...
			this._projectDisposables.push(
				createRefreshFileWatcher(
					this,
					path.join(
						project.options.folder.uri.fsPath,
						"*",
						hostFileName
					)
				)
		...
		for (const unintializedProject of workspaceProjects.unintializedProjects) {
			...
			children.push(
				new InitLocalProjectTreeItem(
					this,
					unintializedProject.projectPath,
					unintializedProject.workspaceFolder
				)
			);
		}
		...
		for (const invalidProject of workspaceProjects.invalidProjects) {
			...
			children.push(
				new InvalidLocalProjectTreeItem(
					this,
					invalidProject.projectPath,
					invalidProject.error,
					invalidProject.workspaceFolder
				)
			);
		}
		...
		if (
			!hasLocalProject &&
			children.length > 0 &&
			children[0] instanceof GenericTreeItem
		) {
	...
	public compareChildrenImpl(
		...
		if (
			item1 instanceof LocalProjectTreeItemBase &&
			!(item2 instanceof LocalProjectTreeItemBase)
		) {
			return 1;
		} else if (
			!(item1 instanceof LocalProjectTreeItemBase) &&
	...
	public async pickTreeItemImpl(
		...
		if (expectedContextValues.some(isProjectCV)) {
			if (
				expectedContextValues.some(isLocalProjectCV) &&
				expectedContextValues.some(isRemoteProjectCV)
			) {
				this.childTypeLabel = localize(
					"projectOrSubscription",
