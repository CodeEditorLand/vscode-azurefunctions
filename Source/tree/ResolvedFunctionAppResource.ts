...
import { type IParsedHostJson, parseHostJson } from "../funcConfig/host";
...
export function isResolvedFunctionApp(
	ti: unknown,
): ti is ResolvedAppResourceBase {
	return (
		(ti as unknown as ResolvedFunctionAppResource).instance ===
		ResolvedFunctionAppResource.instance
	);
}

export class ResolvedFunctionAppResource
	extends ResolvedFunctionAppBase
	implements ResolvedAppResourceBase
{
	...
	public static productionContextValue = "azFuncProductionSlot";
	public static slotContextValue = "azFuncSlot";
	public static flexContextValue = "azFuncFlex";
	...
	public static createResolvedFunctionAppResource(
		context: IActionContext,
		subscription: ISubscriptionContext,
		site: Site,
		isFlex?: boolean,
	): ResolvedFunctionAppResource {
		const resource = new ResolvedFunctionAppResource(
			subscription,
			site,
			isFlex,
		);
	...
	public async setApplicationSetting(
		context: IActionContext,
		key: string,
		value: string,
	): Promise<void> {
	...
	public async loadMoreChildrenImpl(
		...
		if (!this._functionsTreeItem) {
			this._functionsTreeItem =
				await RemoteFunctionsTreeItem.createFunctionsTreeItem(
					context,
					proxyTree,
				);
		}
	...
	public async pickTreeItemImpl(
		...
		for (const expectedContextValue of expectedContextValues) {
			...
			if (typeof expectedContextValue === "string") {
				...
				if (
					DeploymentTreeItem.contextValue.test(expectedContextValue)
				) {
					return this.deploymentsNode;
				}
			} else if (
	...
	public async isReadOnly(context: IActionContext): Promise<boolean> {
		...
		return [runFromPackageKey, "WEBSITE_RUN_FROM_ZIP"].some(
			(key) =>
				appSettings.properties &&
				envUtils.isEnvironmentVariableSet(appSettings.properties[key]),
		);
	}
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		...
		const wizard = new AzureWizard(wizardContext, {
			promptSteps: [
				new DeleteConfirmationStep(confirmationMessage),
