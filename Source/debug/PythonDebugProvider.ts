...
async function getPythonCommand(host: string, port: number): Promise<string> {
	...
	if (pyExtension) {
		...
		if (pyExtension.exports && pyExtension.exports.debug) {
			return (
				await pyExtension.exports.debug.getRemoteLauncherCommand(
					host,
					port,
					false
				)
			...
			throw new Error(
				localize(
					"pyExtOutOfDate",
					'You must update extension with id "{0}" to debug Python projects.',
					pyExtensionId
				)
			);
		}
		...
		throw new Error(
			localize(
				"noPyExt",
				'You must install extension with id "{0}" to debug Python projects.',
				pyExtensionId
			)
		);
	}
