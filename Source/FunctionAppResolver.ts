...
export class FunctionAppResolver implements AppResourceResolver {
		...
		return await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				if (this.siteCacheLastUpdated < Date.now() - 1000 * 3) {
					...
					const sites20231201 = await getSites20231201(
						context,
						subContext,
					);
					await Promise.all(
						sites.map(async (site): Promise<void> => {
							const id = nonNullProp(site, "id").toLowerCase();
							const s = sites20231201.find(
								(s) =>
									s.id?.toLowerCase() ===
									site.id?.toLowerCase(),
							);
				...
				return ResolvedFunctionAppResource.createResolvedFunctionAppResource(
					context,
					subContext,
					nonNullValue(site),
					site?.isFlex,
	...
	public matchesResource(resource: AppResource): boolean {
		return (
			...
			!resource.kind?.includes("workflowapp")
		); // exclude logic apps
	}
}

async function getSites20231201(
...
> {
	try {
		...
		const armEndpoint = ensureEndingSlash(
			subContext.environment.resourceManagerEndpointUrl,
		);
		...
		return (
			((result.parsedBody as { value: unknown }).value as (Site & {
