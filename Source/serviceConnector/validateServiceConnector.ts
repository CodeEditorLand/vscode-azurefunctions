...
import {
	ServiceConnectorTreeItem,
	validateLinker,
} from "@microsoft/vscode-azext-serviceconnector";
...
export async function validateServiceConnector(
	context: IActionContext,
	item?: SlotTreeItem | ServiceConnectorTreeItem
): Promise<void> {
	...
	const activityContext = {
		...
		activityTitle: localize(
			"validate",
			`Validate connection "{0}"`,
			serviceConnectorName
		),
	};

	await validateLinker(
		activityContext,
		item.id,
		item.subscription,
		serviceConnectorName
