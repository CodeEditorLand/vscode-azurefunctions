...
export async function generateLinuxErrorMessages(
	hasPackageManager: boolean,
): Promise<ILinuxErrorMessages> {
	let noPackageManager = "";
	let failedInstall = "";
	let linuxDistroInfo = "";
	...
	if (linuxDistroTag) {
		if (linuxDistroTag.PRETTY_NAME) {
			linuxDistroInfo += localize(
				"linuxDistributionInfoPretty",
				'Note: You are currently running "{0}".',
				linuxDistroTag.PRETTY_NAME,
			);
		} else if (linuxDistroTag.NAME && linuxDistroTag.VERSION) {
			linuxDistroInfo += localize(
				"linuxDistributionInfo",
				'Note: You are currently running "{0} {1}".',
				linuxDistroTag.NAME,
				linuxDistroTag.VERSION,
			);
		}
