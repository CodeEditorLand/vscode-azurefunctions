...
import { type FuncVersion, promptForFuncVersion } from "../FuncVersion";
...
import {
	funcToolsInstalled,
	getInstallUrl,
} from "./validateFuncCoreToolsInstalled";

export async function installOrUpdateFuncCoreTools(
	...
	if (packageManagers.length === 0) {
		...
		throw new Error(
			localize(
				"installNotSupported",
				"Failed to install or update. Follow [these instructions]({0}) to install manually.",
				getInstallUrl(),
			),
		);
	}
	...
	if (isFuncInstalled) {
		...
		if (packageManagers.length === 1) {
			...
			const picks: IAzureQuickPickItem<PackageManager>[] =
				packageManagers.map((pm) => {
					return {
						label: localize("update", "Update {0} package", pm),
						data: pm,
					};
				});
			packageManager = (
		...
		if (version === undefined) {
			version = await promptForFuncVersion(
				...
				localize(
					"selectLocalVersion",
					"Failed to detect local version automatically. Select your version to update",
				),
			);
		}
