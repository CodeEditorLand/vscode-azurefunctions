...
import { type MessageItem, env } from "vscode";
import { type FuncVersion, tryParseFuncVersion } from "../FuncVersion";
import { type PackageManager, funcVersionSetting } from "../constants";
...
export async function validateFuncCoreToolsInstalled(
	...
	await callWithTelemetryAndErrorHandling(
		"azureFunctions.validateFuncCoreToolsInstalled",
		async (innerContext: IActionContext) => {
			...
			if (
				...
				if (os.platform() === "linux") {
					const linuxErrorMessages: ILinuxErrorMessages =
...
export async function funcToolsInstalled(
	context: IActionContext,
