			...
			if (showCoreToolsWarning || showMultiCoreToolsWarning) {
				...
				if (showCoreToolsWarning) {
					...
					const newestVersion: string | undefined =
						await getNewestFunctionRuntimeVersion(
							packageManager,
							versionFromSetting,
							context,
						);
					...
					if (
						semver.major(newestVersion) ===
							semver.major(localVersion) &&
						...
						do {
							...
							if (result === DialogResponses.learnMore) {
								...
								await updateFuncCoreTools(
									context,
									packageManager,
									versionFromSetting,
								);
							} else if (
								result === DialogResponses.dontWarnAgain
							) {
								await updateGlobalSetting(
									showCoreToolsWarningKey,
									false,
								);
							}
...
async function getNewestFunctionRuntimeVersion(
	...
	try {
		...
		context.telemetry.properties.latestRuntimeError = maskUserInfo(
