...
namespace DotnetDebugDebugConfiguration {
	...
	): debugConfiguration is DotnetDebugDebugConfiguration {
		return (
			debugConfiguration.type === "coreclr" &&
			"launchServiceData" in debugConfiguration
		);
	}
}
...
class RunningFunctionTaskMap {
	...
	public set(
		key: vscode.WorkspaceFolder | vscode.TaskScope,
	...
	public get(
		key: vscode.WorkspaceFolder | vscode.TaskScope,
		buildPath?: string,
	): IRunningFuncTask | undefined {
		const values = this._map.get(key) || [];

		return values.find((t) => {
			const taskExecution = t.taskExecution.task
			...
			const taskDirectory = taskExecution.options?.cwd?.replace(
				"${workspaceFolder}",
			...
			return (
				taskDirectory &&
				buildPath &&
				normalizePath(taskDirectory) === normalizePath(buildPath)
			);
		});
	}
...
export const runningFuncTaskMap: RunningFunctionTaskMap =
	new RunningFunctionTaskMap();
...
export function registerFuncHostTaskEvents(): void {
	registerEvent(
		...
		async (context: IActionContext, e: vscode.TaskProcessStartEvent) => {
			...
			if (
				...
				isFuncHostTask(e.execution.task)
			) {
				const portNumber = await getFuncPortFromTaskOrProject(
					context,
					e.execution.task,
					e.execution.task.scope,
				);
	...
	registerEvent(
		...
		(context: IActionContext, e: vscode.TaskProcessEndEvent) => {
			...
			if (
				...
				runningFuncTaskMap.delete(
					e.execution.task.scope,
					(e.execution.task.execution as vscode.ShellExecution)
						.options?.cwd,
				);
			}
		},
	...
	registerEvent(
		"azureFunctions.onDidTerminateDebugSession",
		...
		(context: IActionContext, debugSession: vscode.DebugSession) => {
			...
			if (
				DotnetDebugDebugConfiguration.is(debugSession.configuration) &&
				debugSession.configuration.launchServiceData.buildPath
			) {
				const buildPathUri: vscode.Uri = vscode.Uri.file(
					debugSession.configuration.launchServiceData.buildPath,
				);
				...
				if (workspaceFolder === undefined) {
					throw new Error(
						localize(
							"noWorkspaceFolderForBuildPath",
							'No workspace folder found for path "{0}".',
							buildPathUri.fsPath,
						),
					);
				}

				stopFuncTaskIfRunning(
					workspaceFolder,
					buildPathUri.fsPath,
					false,
					true,
				...
				buildPathToWorkspaceFolderMap.delete(
					debugSession.configuration.launchServiceData.buildPath,
				);
			}
			...
			if (
				getWorkspaceSetting<boolean>("stopFuncTaskPostDebug") &&
...
export function stopFuncTaskIfRunning(
...
export async function getFuncPortFromTaskOrProject(
	...
	try {
		...
		if (funcTask && funcTask.execution instanceof vscode.ShellExecution) {
			const match = funcTask.execution?.commandLine?.match(
				/\s+(?:"|'|)(?:-p|--port)(?:"|'|)\s+(?:"|'|)([0-9]+)/i,
			);
		...
		if (typeof projectPathOrTaskScope === "string") {
			...
			projectPath = await tryGetFunctionProjectRoot(
				context,
				projectPathOrTaskScope,
			);
		}

		if (projectPath) {
			const localSettings = await getLocalSettingsJson(
				context,
				path.join(projectPath, localSettingsFileName),
			);

			if (localSettings.Host) {
				const key = Object.keys(localSettings.Host).find(
					(k) => k.toLowerCase() === "localhttpport",
				);
...
function normalizePath(fsPath: string): string {
	return vscode.Uri.parse(
		path.normalize(fsPath).replace(/^(\/|\\)+|(\/|\\)+$/g, ""),
	).fsPath;
}
