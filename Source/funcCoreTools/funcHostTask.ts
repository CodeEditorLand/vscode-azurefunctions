...
export const runningFuncTaskMap: Map<
	vscode.WorkspaceFolder | vscode.TaskScope,
	IRunningFuncTask
> = new Map<vscode.WorkspaceFolder | vscode.TaskScope, IRunningFuncTask>();

const funcTaskStartedEmitter = new vscode.EventEmitter<
	vscode.WorkspaceFolder | vscode.TaskScope | undefined
>();
export const onFuncTaskStarted = funcTaskStartedEmitter.event;

export const runningFuncPortMap = new Map<
	vscode.WorkspaceFolder | vscode.TaskScope | undefined,
	string
>();
	...
	registerEvent(
		...
		async (context: IActionContext, e: vscode.TaskProcessStartEvent) => {
			...
			if (
				...
				runningFuncPortMap.set(
					e.execution.task.scope,
					await getFuncPortFromTaskOrProject(
						context,
						e.execution.task,
						e.execution.task.scope,
					),
...
export function stopFuncTaskIfRunning(
	...
	const runningFuncTask: IRunningFuncTask | undefined =
...
export async function getFuncPortFromTaskOrProject(
	...
	try {
		...
		if (funcTask && typeof funcTask.definition.command === "string") {
			const match = funcTask.definition.command.match(
				/\s+(?:"|'|)(?:-p|--port)(?:"|'|)\s+(?:"|'|)([0-9]+)/i,
			);
			if (match) {
				return match[1];
