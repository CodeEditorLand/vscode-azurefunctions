...
export async function uninstallFuncCoreTools(
	...
	if (packageManagers.length === 0) {
		throw new Error(
			localize(
				"notInstalled",
				"Cannot uninstall Azure Functions Core Tools because it is not installed with brew or npm."
			)
		);
		...
		const picks: IAzureQuickPickItem<PackageManager>[] =
			packageManagers.map((pm) => {
				return {
					label: localize("uninstall", "Uninstall {0} package", pm),
					data: pm,
				};
			});
		packageManager = (
